...
public:
    ...
    virtual ~THashObj()
    {
        if (priorHash && priorHash != this) UnlinkHash();
    }
...
public:
    ...
    void	SetUsedSize(int _used_size)
    {
        usedSize = _used_size;
    }
    int		AddUsedSize(int _used_size)
    {
        return usedSize += _used_size;
    }
    int		RemainSize(void)
    {
