/*$T indentinput.cpp GC 1.140 12/29/10 17:26:05 */
...
void VECTOR2D_Print(VECTOR2D_PTR va, char *name = "v")
{
	...
	Write_Error("\n%s=[", name);
	for(int index = 0; index < 2; index++) Write_Error("%f, ", va->M[index]);
	Write_Error("]");
}	/* end VECTOR2D_Print */
...
void VECTOR3D_Print(VECTOR3D_PTR va, char *name = "v")
{
	...
	Write_Error("\n%s=[", name);
	for(int index = 0; index < 3; index++) Write_Error("%f, ", va->M[index]);
	Write_Error("]");
}	/* end VECTOR3D_Print */
...
void VECTOR4D_Print(VECTOR4D_PTR va, char *name = "v")
{
	...
	Write_Error("\n%s[", name);
	for(int index = 0; index < 4; index++) Write_Error("%f, ", va->M[index]);
	Write_Error("]");
}	/* end VECTOR4D_Print */
...
void Print_Mat_2X2(MATRIX2X2_PTR ma, char *name = "M")
{
	...
	for(int r = 0; r < 2; r++, Write_Error("\n"))
		for(int c = 0; c < 2; c++) Write_Error("%f ", ma->M[r][c]);
}	/* end Print_Mat_2X2 */
...
int Solve_2X2_System(MATRIX2X2_PTR A, MATRIX1X2_PTR X, MATRIX1X2_PTR B)
{
	...
	if(fabs(det_A) < EPSILON_E5) return(0);
	...
	/* solve for x copy A into working matrix */
	...
	/* solve for y copy A into working matrix */
	MAT_COPY_2X2(A, &work_mat);
...
int Mat_Inverse_3X3(MATRIX3X3_PTR m, MATRIX3X3_PTR mi)
{
	 ...
	 * an inverse
	 */
	float	det = m->M00 * (m->M11 * m->M22 - m->M21 * m->M12) - m->M01 * (m->M10 * m->M22 - m->M20 * m->M12) + m->M02 *
...
void Print_Mat_3X3(MATRIX3X3_PTR ma, char *name = "M")
{
	...
	for(int r = 0; r < 3; r++, Write_Error("\n"))
		for(int c = 0; c < 3; c++) Write_Error("%f ", ma->M[r][c]);
}	/* end Print_Mat_3X3 */
...
float Mat_Det_3X3(MATRIX3X3_PTR m)
{
	...
	return
		(
			m->M00 * (m->M11 * m->M22 - m->M21 * m->M12) - m->M01 * (m->M10 * m->M22 - m->M20 * m->M12) + m->M02 *
				(m->M10 * m->M21 - m->M20 * m->M11)
		);
}	/* end Mat_Det_3X3 */
...
int Solve_3X3_System(MATRIX3X3_PTR A, MATRIX1X3_PTR X, MATRIX1X3_PTR B)
{
	...
	/* solve for z copy A into working matrix */
	MAT_COPY_3X3(A, &work_mat);
...
void Mat_Mul_VECTOR4D_4X4(VECTOR4D_PTR va, MATRIX4X4_PTR mb, VECTOR4D_PTR vprod)
{
	 ...
	 * 4x4 matrix - ma*mb and stores the result in mprod ;
	 * the function makes no assumption
	 */
	for(int col = 0; col < 4; col++)
	{
...
void Print_Mat_4X4(MATRIX4X4_PTR ma, char *name = "M")
{
	...
	for(int r = 0; r < 4; r++, Write_Error("\n"))
		for(int c = 0; c < 4; c++) Write_Error("%f ", ma->M[r][c]);
}	/* end Print_Mat_4X4 */
...
int Intersect_Parm_Lines2D(PARMLINE2D_PTR p1, PARMLINE2D_PTR p2, float *t1, float *t2)
{
	...
	if(fabs(det_p1p2) <= EPSILON_E5)
	{
		 ...
		 * consideration the overlap and singular point exception
		 */
...
float Compute_Point_In_Plane3D(POINT3D_PTR pt, PLANE3D_PTR plane)
{
	 ...
	 */
	float	hs = plane->n.x * (pt->x - plane->p0.x) + plane->n.y * (pt->y - plane->p0.y) + plane->n.z *
		(pt->z - plane->p0.z);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
int Intersect_Parm_Line3D_Plane3D(PARMLINE3D_PTR pline, PLANE3D_PTR plane, float *t, POINT3D_PTR pt)
{
	 ...
	 * d = (-a*xp0 - b*yp0 - c*zp0), xp0, yp0, zp0, define the point on the plan
	 */
	*t = -
		(
			plane->n.x *
