/*$T indentinput.cpp GC 1.140 12/29/10 17:25:25 */
...
/*
 * DEFINES GLOBALS ;
 * this array is used to quickly compute the nearest power of 2 for a number from
 * 0-256 ;
...
/*
 =======================================================================================================================
    FUNCTIONS setting the frame is so important that it should be a member function
 =======================================================================================================================
 */
int OBJECT4DV2::Set_Frame(int frame)
{
	 ...
	 * pointers based on the head pointer
	 */
...
int Set_OBJECT4DV2_Frame(OBJECT4DV2_PTR obj, int frame)
{
	...
	/* test if its multiframe? */
	if(!(obj->attr & OBJECT4DV2_ATTR_MULTI_FRAME)) return(0);
...
int Destroy_OBJECT4DV2(OBJECT4DV2_PTR obj)	/* object to destroy */
{
	 ...
	 * local vertex list \
	 */
	if(obj->head_vlist_local) free(obj->head_vlist_local);

	/* transformed vertex list */
	if(obj->head_vlist_trans) free(obj->head_vlist_trans);

	/* texture coordinate list */
	if(obj->tlist) free(obj->tlist);

	/* polygon list */
	if(obj->plist) free(obj->plist);

	/* object radii arrays */
	if(obj->avg_radius) free(obj->avg_radius);
	if(obj->max_radius) free(obj->max_radius);
...
int Init_OBJECT4DV2
(
	OBJECT4DV2_PTR	obj,	/* object to allocate */
	int				_num_vertices,
	int				_num_polys,
	int				_num_frames,
	int				destroy
)
{
	 ...
	 */
	if(destroy) Destroy_OBJECT4DV2(obj);

	/* allocate memory for vertex lists */
	if(!(obj->vlist_local = (VERTEX4DTV1_PTR) malloc(sizeof(VERTEX4DTV1) * _num_vertices * _num_frames))) return(0);

	/* clear data */
	memset((void *) obj->vlist_local, 0, sizeof(VERTEX4DTV1) * _num_vertices * _num_frames);

	if(!(obj->vlist_trans = (VERTEX4DTV1_PTR) malloc(sizeof(VERTEX4DTV1) * _num_vertices * _num_frames))) return(0);
	...
	if(!(obj->tlist = (POINT2D_PTR) malloc(sizeof(POINT2D) * _num_polys * 3))) return(0);

	/* clear data */
	memset((void *) obj->tlist, 0, sizeof(POINT2D) * _num_polys * 3);

	/* allocate memory for radii arrays */
	if(!(obj->avg_radius = (float *) malloc(sizeof(float) * _num_frames))) return(0);

	/* clear data */
	memset((void *) obj->avg_radius, 0, sizeof(float) * _num_frames);

	if(!(obj->max_radius = (float *) malloc(sizeof(float) * _num_frames))) return(0);

	/* clear data */
	memset((void *) obj->max_radius, 0, sizeof(float) * _num_frames);

	/* allocate memory for polygon list */
	if(!(obj->plist = (POLY4DV2_PTR) malloc(sizeof(POLY4DV2) * _num_polys))) return(0);
...
void Transform_OBJECT4DV2
(
	OBJECT4DV2_PTR						obj,			/* object to transform */
	MATRIX4X4_PTR						mt,				/* transformation matrix */
	int									coord_select,	/* selects coords to transform */
	int									transform_basis,	/* flags if vector orientation */
	/* should be transformed too */int	all_frames
)	/* should all frames be transformed */
{
...
void Rotate_XYZ_OBJECT4DV2
(
	OBJECT4DV2_PTR	obj,		/* object to rotate */
...
int Cull_OBJECT4DV2
(
	OBJECT4DV2_PTR	obj,	/* object to cull */
...
)	/* clipping planes to consider */
{
	...
	if(cull_flags & CULL_OBJECT_Z_PLANE)
	{
		...
		if
		(
			((sphere_pos.z - obj->max_radius[obj->curr_frame]) > cam->far_clip_z)
	...
	if(cull_flags & CULL_OBJECT_X_PLANE)
	{
		...
		if
		(
			((sphere_pos.x - obj->max_radius[obj->curr_frame]) > z_test)	/* right side */
		||	((sphere_pos.x + obj->max_radius[obj->curr_frame]) < -z_test)
		)	/* left side, note sign change */
		{
			SET_BIT(obj->state, OBJECT4DV2_STATE_CULLED);
			return(1);
		}	/* end if */
	...
	if(cull_flags & CULL_OBJECT_Y_PLANE)
	{
		...
		if
		(
			((sphere_pos.y - obj->max_radius[obj->curr_frame]) > z_test)	/* top side */
		||	((sphere_pos.y + obj->max_radius[obj->curr_frame]) < -z_test)
		)	/* bottom side, note sign change */
		{
			SET_BIT(obj->state, OBJECT4DV2_STATE_CULLED);
			return(1);
		}	/* end if */
...
void Remove_Backfaces_RENDERLIST4DV2(RENDERLIST4DV2_PTR rend_list, CAM4DV1_PTR cam)
{
	...
	for(int poly = 0; poly < rend_list->num_polys; poly++)
	{
		 ...
		 * active, and visible and not 2 sided. Note we test for backface in the event
		 * that ;
		...
		if(dp <= 0.0) SET_BIT(curr_poly->state, POLY4DV2_STATE_BACKFACE);
	}	/* end for poly */
...
void Remove_Backfaces_OBJECT4DV2(OBJECT4DV2_PTR obj, CAM4DV1_PTR cam)
{
	 ...
	 */
	if(obj->state & OBJECT4DV2_STATE_CULLED) return;
...
void Camera_To_Perspective_OBJECT4DV2(OBJECT4DV2_PTR obj, CAM4DV1_PTR cam)
{
	...
	for(int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		 ...
		 * z = z, so no change ;
		 * not that we are NOT dividing by the homogenous w coordinate since ;
		 * we are not using a matrix operation for this version of the functio
		 */
	}	/* end for vertex */
...
int Insert_POLY4DV2_RENDERLIST4DV2(RENDERLIST4DV2_PTR rend_list, POLY4DV2_PTR poly)
{
	 ...
	 * step 0: are we full?
	 */
	if(rend_list->num_polys >= RENDERLIST4DV2_MAX_POLYS) return(0);
	...
	/*
	 * copy fields { ??????????? make sure ALL fields are copied, normals, textures,
	 * etc!!! }
	 */
...
int Insert_OBJECT4DV2_RENDERLIST4DV2(RENDERLIST4DV2_PTR rend_list, OBJECT4DV2_PTR obj, int insert_local = 0)
{
...
void Reset_OBJECT4DV2(OBJECT4DV2_PTR obj)
{
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		/* first is this polygon even visible? */
		if(!(curr_poly->state & POLY4DV2_STATE_ACTIVE)) continue;	/* move onto next poly */
...
int Compare_AvgZ_POLYF4DV2(const void *arg1, const void *arg2)
{
	 ...
	 * depth sort surface ordering algorith
	 */
	float			z1, z2;
...
)	/* maximum lights in list */
{
	 ...
	 * function lights an object based on the sent lights and camera. the function
	 * supports ;
	 * constant/pure shading (emmisive), flat shading with ambient, infinite, point
	 * lights, and spot lights ;
	 * note that this lighting function is rather brute force and simply follows the
	 * math, however ;
	 * there are some clever integer operations that are used in scale 256 rather than
	 * going to floating ;
	 * point, but why? floating point and ints are the same speed, HOWEVER, the
	 * conversion to and from floating ;
	 * point can be cycle intensive, so if you can keep your calcs in ints then you
	 * can gain some speed ;
	 * also note, type 1 spot lights are simply point lights with direction, the
	 * "cone" is more of a function ;
	 * of the falloff due to attenuation, but they still look like spot lights ;
	 * type 2 spot lights are implemented with the intensity having a dot product
	 * relationship with the ;
	 * angle from the surface point to the light direction just like in the optimized
	 * model, but the pf term ;
	 * that is used for a concentration control must be 1,2,3,.... integral and
	 * non-fractional
	 */
	unsigned int	r_base, g_base, b_base, /* base color being lit */
	r_sum, g_sum, b_sum,	/* sum of lighting process over all lights */
	r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2, g_sum2, b_sum2, ri, gi, bi, shaded_color;	/* final color */
	float			dp, /* dot product */
	dist,		/* distance from light to surface */
	dists, i,	/* general intensities */
	nl,			/* length of normal */
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
		{
			 ...
			 * the max value, if they are the max value then the first light that needs the
			 * math ;
			 * will do it, and then save the information into the variable (causing it to
			 * change state ;
			 * from an invalid number) then any other lights that need the math can use the
			 * previously ;
			 * computed value ;
			...
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
			{
				...
				if(lights[curr_light].state == LIGHTV1_STATE_OFF) continue;
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_INFINITE)
				{
					 ...
					 * this is a key optimization we can make later, we can pre-compute the length of
					 * all polygon ;
					 ...
					 * notice the scaling by 128, I want to avoid floating point calculations, not
					 * because they ;
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_POINT)
				{
					...
					if(dp > 0)
					{
						atten =
							(
								lights[curr_light].kc +
								lights[curr_light].kl *
								dist +
								lights[curr_light].kq *
								dist *
								dist
							);
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_SPOTLIGHT1)
				{
					 ...
					 * because they ;
					 * are slower, but the conversion to and from cost cycles ;
					 * note that I use the direction of the light here rather than a the vector to the
					 * light ;
					 * thus we are taking orientation into account which is similar to the spotlight
					 * model
					 */
					dp = VECTOR4D_Dot(&n, &lights[curr_light].dir);
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_SPOTLIGHT2)	/* simple version */
				{
					 ...
					 * thus it's almost identical to the point, but has the extra term in the
					 * numerator ;
					...
					if(dp > 0)
					{
						...
						if(dpsl > 0)
						{
							...
							/*~~~~~~~~~~~~~~~~~~~~*/
							/*
							 * for speed reasons, pf exponents that are less that 1.0 are out of the question,
							 * and exponents ;
							 * must be integral
							 */
							float	dpsl_exp = dpsl;
							...
							for(int e_index = 1; e_index < (int) lights[curr_light].pf; e_index++) dpsl_exp *= dpsl;
			...
			/*
			 * Write_Error("\nWriting final values to polygon %d = %d,%d,%d", poly, r_sum,
			 * g_sum, b_sum);
		...
		else if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
		{
			...
			/*
			 * gouraud shade, unfortunetly at this point in the pipeline, we have lost the
			 * original ;
			 * mesh, and only have triangles, thus, many triangles will share the same
			 * vertices and ;
			 * they will get lit 2x since we don't have any way to tell this, alas, performing
			 * lighting ;
			 * at the object level is a better idea when gouraud shading is performed since
			 * the ;
			 * commonality of vertices is still intact, in any case, lighting here is similar
			 * to polygon ;
			 * flat shaded, but we do it 3 times, once for each vertex, additionally there are
			 * lots ;
			 * of opportunities for optimization, but I am going to lay off them for now, so
			 * the code ;
			...
			/*
			 * Write_Error("\nColor sums rgb0[%d, %d, %d], rgb1[%d,%d,%d], rgb2[%d,%d,%d]",
			 * r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2, g_sum2, b_sum2);
			 * * new optimization: ;
			 * when there are multiple lights in the system we will end up performing numerous ;
			 * redundant calculations to minimize this my strategy is to set key variables to ;
			 * to MAX values on each loop, then during the lighting calcs to test the vars for ;
			...
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
			{
				...
				if(lights[curr_light].attr & LIGHTV1_ATTR_AMBIENT)
				{
					...
					/*
					 * there better only be one ambient light! ;
					 * Write_Error("\nexiting ambient ,sums rgb0[%d, %d, %d], rgb1[%d,%d,%d],
					 * rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2,
					 * g_sum2, b_sum2);
					 */
				}		/* end if */
				else if(lights[curr_light].attr & LIGHTV1_ATTR_INFINITE)
				{
					 ...
					 * because they ;
					 * are slower, but the conversion to and from cost cycles ;
					 * need to perform lighting for each vertex (lots of redundant math, optimize
					 * later!) ;
					 * Write_Error("\nv0=[%f, %f, %f]=%f, v1=[%f, %f, %f]=%f, v2=[%f, %f, %f]=%f", ;
					 * curr_poly->tvlist[0].n.x, curr_poly->tvlist[0].n.y,curr_poly->tvlist[0].n.z,
					 * VECTOR4D_Length(&curr_poly->tvlist[0].n), ;
					 * curr_poly->tvlist[1].n.x, curr_poly->tvlist[1].n.y,curr_poly->tvlist[1].n.z,
					 * VECTOR4D_Length(&curr_poly->tvlist[1].n), ;
					 * curr_poly->tvlist[2].n.x, curr_poly->tvlist[2].n.y,curr_poly->tvlist[2].n.z,
					 * VECTOR4D_Length(&curr_poly->tvlist[2].n) );
					 * ;
					 * vertex 0
					 */
					...
					/*
					 * Write_Error("\nexiting infinite, color sums rgb0[%d, %d, %d], rgb1[%d,%d,%d],
					 * rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2,
					 * g_sum2, b_sum2);
					 */
				}		/* end if infinite light */
				else if(lights[curr_light].attr & LIGHTV1_ATTR_POINT)
				{
					...
					/*
					 * Write_Error("\nexiting point light, rgb0[%d, %d, %d], rgb1[%d,%d,%d],
					 * rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2,
					 * g_sum2, b_sum2);
					 */
				}		/* end if point */
				else if(lights[curr_light].attr & LIGHTV1_ATTR_SPOTLIGHT1)
				{
					...
					/*
					 * Write_Error("\nexiting spotlight1, sums rgb0[%d, %d, %d], rgb1[%d,%d,%d],
					 * rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2,
					 * g_sum2, b_sum2);
					 */
				}		/* end if spotlight1 */
				else if(lights[curr_light].attr & LIGHTV1_ATTR_SPOTLIGHT2)	/* simple version */
				{
					...
					/*
					 * Write_Error("\nexiting spotlight2, sums rgb0[%d, %d, %d], rgb1[%d,%d,%d],
					 * rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2,
					 * g_sum2, b_sum2);
					 */
				}			/* end if spot light */
			 ...
			 * *Write_Error("\n final sums rgb0[%d, %d, %d], rgb1[%d,%d,%d], rgb2[%d,%d,%d]",
			 * r_sum0, g_sum0, b_sum0, r_sum1, g_sum1, b_sum1, r_sum2, g_sum2, b_sum2);
			 * ;
			 * write the colors
			 */
...
)	/* maximum lights in list */
{
	 ...
	 * that is used for a concentration control must be 1,2,3,.... integral and
	 * non-fractional ;
	 * the function works in 8-bit color space, and uses the rgblookup[] to look
	 * colors up from RGB values ;
	 * basically, the function converts the 8-bit color index into an RGB value
	 * performs the lighting ;
	 * operations and then back into an 8-bit color index with the table, so we loose
	 * a little bit during the incoming and ;
	 * outgoing transformations, however, the next step for optimization will be to
	 * make a purely monochromatic ;
	 * 8-bit system that assumes ALL lights are white, but this function works with
	 * color for now
	 */
	unsigned int	r_base, g_base, b_base, /* base color being lit */
	r_sum, g_sum, b_sum,	/* sum of lighting process over all lights */
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
		{
			...
			/*
			 * Write_Error("\nEntering Flat Shader");
			 * * step 1: extract the base color out in RGB mode (it's already in 8 bits per
			 * channel)
			 */
...
(
	RENDERLIST4DV2_PTR	rend_list,	/* list to process */
	CAM4DV1_PTR			cam,		/* camera position */
	LIGHTV1_PTR			lights,		/* light list (might have more than one) */
	int					max_lights
)	/* maximum lights in list */
{
	...
	/*
	 * 16-bit version of function ;
	 * function lights the entire rendering list based on the sent lights and camera.
	 ...
	 * that is used for a concentration control must be 1,2,3,.... integral and
	 * non-fractional ;
	 * this function now performs emissive, flat, and gouraud lighting, results are
	 * stored in the ;
	...
	for(int poly = 0; poly < rend_list->num_polys; poly++)
	{
		...
		else if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
		{
			 ...
			 * from an invalid number) then any other lights that need the math can use the
			 * previously ;
			 * computed value, however, since we already have the normals, not much here to
			 * cache on ;
			 * a large scale, but small scale stuff is there, however, we will optimize those
			 * later ;
			 * loop thru lights
			 */
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
...
)	/* maximum lights in list */
{
	 ...
	 * that is used for a concentration control must be 1,2,3,.... integral and
	 * non-fractional ;
	 * also note since we are dealing with a rendering list and not object, the final
	 * lit color is ;
	...
	for(int poly = 0; poly < rend_list->num_polys; poly++)
	{
		...
		if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
		{
			 ...
			 * previously ;
			 * computed value ;
			 * set surface normal.z to FLT_MAX to flag it as non-compute
			 */
			n.z = FLT_MAX;
		...
		else if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
		{
			...
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
			{
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_SPOTLIGHT1)
				{
					 ...
					 * Write_Error("\nentering spotlight1....");
					 * * .. normal is already computed ;
					 * compute vector from surface to ligh
					 */
					VECTOR4D_Build(&curr_poly->tvlist[0].v, &lights[curr_light].pos, &l);
...
float Compute_OBJECT4DV2_Radius(OBJECT4DV2_PTR obj)
{
	...
	for(int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		if(dist_to_vertex > obj->max_radius[obj->curr_frame]) obj->max_radius[obj->curr_frame] = dist_to_vertex;
	}	/* end for vertex */
...
char *Extract_Filename_From_Path(char *filepath, char *filename)
{
	 ...
	 * test of filepath is vali
	 */
	if(!filepath || strlen(filepath) == 0) return(NULL);
	...
	/* find filename */
	while((filepath[index_end] != '\\') && (filepath[index_end] != '/') && (filepath[index_end] > 0)) index_end--;
...
/* and perform transforms */
{
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*
	 * this function loads a Caligari TrueSpace .COB file object in off disk,
	 * additionally ;
	 ...
	 * also, since this version IS going to read in the texture map and texture
	 * coordinates ;
	 * we have a couple issues to think about, first COB format like absolute texture
	 * paths ;
	 * we can't have that, so we will simple extract out ONLY the texture map bitmap
	 * name ;
	 * and use the global texture path variable to build a real file path, also
	 * texture ;
	 * coordinates are in 0..1 0..1 form, I still haven't decided if I want to use
	 * absolute ;
	...
	while(1)
	{
		...
		if(parser.Pattern_Match(parser.buffer, "['Transform']"))
		{
			...
			/*
			 * no need to read in last row, since it's always 0,0,0,1 and we don't use it
			 * anyway
			 */
			break;
		}		/* end if */
	...
	/*
	 * allocate the memory for the vertices and number of polys (unknown, so use
	 * 3*num_vertices) ;
	 * the call parameters are redundant in this case, but who cares
	 */
	if(!Init_OBJECT4DV2(obj, /* object to allocate */ obj->num_vertices, obj->num_vertices * 3, obj->num_frames))
	{
	...
	for(int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		while(1)
		{
			...
			if(parser.Pattern_Match(parser.buffer, "[f] [f] [f]"))
			{
				 ...
				 * #define VERTEX_FLAGS_INVERT_WINDING_ORDER 64 // invert winding order from cw to
				 * ccw or ccw to cc ;
				 * #define VERTEX_FLAGS_TRANSFORM_LOCAL 512 // if file format has local transform
				 * then do it! ;
				 * #define VERTEX_FLAGS_TRANSFORM_LOCAL_WORLD 1024 // if file format has local to
				 * world then do it!
				 */
				...
				/* invert signs? */
				if(vertex_flags & VERTEX_FLAGS_INVERT_X) obj->vlist_local[vertex].x = -obj->vlist_local[vertex].x;
				if(vertex_flags & VERTEX_FLAGS_INVERT_Y) obj->vlist_local[vertex].y = -obj->vlist_local[vertex].y;
				if(vertex_flags & VERTEX_FLAGS_INVERT_Z) obj->vlist_local[vertex].z = -obj->vlist_local[vertex].z;
	...
	for(; tvertex < num_texture_vertices; tvertex++)
	{
	...
	/*
	 * when we load in the polygons then we will copy the texture vertices into the
	 * polygon ;
	 * vertices assuming that each vertex has a SINGLE texture coordinate, this means
	 * that ;
	 * you must NOT use multiple textures on an object! in other words think "skin"
	 * this is ;
	 * inline with Quake II md2 format, in 99% of the cases a single object can be
	 * textured ;
	 * with a single skin and the texture coordinates can be unique for each vertex
	 * and 1:1
	 */
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		while(1)
		{
			...
			if(parser.Pattern_Match(parser.buffer, "['Face'] ['verts'] [i] ['flags'] [i] ['mat'] [i]"))
			{
				...
				/*
				 * at this point we have the number of vertices for the polygon, the flags, and
				 * it's material index ;
				 * in the integer output array locations 0,1,2 ;
				 * store the material index for this polygon for retrieval later, but make sure
				 * adjust the ;
				 * the index to take into consideration that the data in parser.pints[2] is 0
				 * based, and we need ;
				 * an index relative to the entire library, so we simply need to add num_materials
				 * to offset the ;
				 * index properly, but we will leave this reference zero based for now... and fix
				 * up later
				 */
	...
	for(; curr_material < num_materials_object; curr_material++)
	{
		...
		while(1)
		{
			...
			if(parser.Pattern_Match(parser.buffer, "['mat#'] [i]"))
			{
				...
				/*
				 * extract out lighting constants for the heck of it, they are on a line like
				 * this: ;
				 ...
				 * although our engine will have minimal support for these, we might as well get
				 * them
				 */
				while(1)
				{
				...
				/*
				 * now we need to know the shading model, it's a bit tricky, we need to look for
				 * the lines ;
				 ...
				 * THEN based on that we hunt for "Shader class: reflectance" which is where the
				 * type ;
				 ...
				 * and in the case that in the "color" class, we found a "texture map" then the
				 * "shading mode" is ;
					...
					if(parser.Pattern_Match(parser.buffer, "['Shader'] ['name:'] ['texture'] ['map']"))
					{
						...
						while(1)
						{
							...
							if(parser.Pattern_Match(parser.buffer, "['file'] ['name:'] ['string']"))
							{
								 ...
								 * filename and path starts at char position 19, 0 indexe
								 */
								memcpy
								(
									materials[material_index + num_materials].texture_file,
									&parser.buffer[18],
								 ...
								 * the materials, if this is the first texture map, load it, and set a flag
								 * disallowing ;
				...
				/*
				 * alright, finally! Now we need to know what the actual shader type, now in the
				 * COB format ;
				 * I have decided that in the "reflectance" class that's where we will look at
				 * what kind ;
	...
	/*
	 * at this point poly_material[] holds all the indices for the polygon materials
	 * (zero based, so they need fix up) ;
	 * and we must access the materials array to fill in each polygon with the polygon
	 * color, etc. ;
	...
	for(int curr_poly = 0; curr_poly < obj->num_polys; curr_poly++)
	{
		Write_Error
		(
			"\nfixing poly material %d from index %d to index %d",
			curr_poly,
		...
		else
		{
			...
			{
				obj->plist[curr_poly].color = RGBto8BitIndex
					(
						materials[poly_material[curr_poly]].color.r,
						materials[poly_material[curr_poly]].color.g,
						materials[poly_material[curr_poly]].color.b,
	...
	/*
	 * local object materials have been added to database, update total materials in
	 * system
	 */
	...
	/*
	 * now that we know the correct number of polygons, we must allocate memory for
	 * them ;
	 * and fix up the object, this is a hack, but the file formats are so stupid by
	 * not ;
	 ...
	 */
	if(!(plist_temp = (POLY4DV2_PTR) malloc(sizeof(POLY4DV2) * obj->num_polys))) return(0);
...
/* and shading overrides */
{
	...
	/*
	 * Step 3: ;
	 * lets find the name of the object firs
	 */
	while(1)
	{
	 ...
	 * the call parameters are redundant in this case, but who cares
	 */
	if(!Init_OBJECT4DV2(obj, /* object to allocate */ obj->num_vertices, obj->num_polys, obj->num_frames))
	{
		Write_Error("\nASC file error with file %s (can't allocate memory).", filename);
	}			/* end if */
	...
	for(int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		while(1)
		{
			...
			/*
			 * strip all ":XYZ", make this easier, note use of input and output as same var,
			 * this is legal ;
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		while(1)
		{
			...
			/*
			 * strip all ":ABC", make this easier, note use of input and output as same var,
			 * this is legal ;
			 * since the output is guaranteed to be the same length or shorter as the input :)
			 */
			...
			if(parser.Pattern_Match(parser.buffer, "['Face'] [i] [i] [i] [i]"))
			{
				...
				/*
				 * at this point we have the vertex indices in the the pints array locations
				 * 1,2,3, ;
		...
		while(1)
		{
			...
			/*
			 * Material:"rdddgdddbddda0" ;
			 * replace all ':"rgba', make this easier, note use of input and output as same
			 * var, this is legal ;
			 * since the output is guaranteed to be the same length or shorter as the input :) ;
			...
			if(parser.Pattern_Match(parser.buffer, "[i] [i] [i]"))
			{
				...
				/*
				 * at this point we have the red, green, and blue components in the the pints
				 * array locations 0,1,2,
				 */
				r = parser.pints[0];
				g = parser.pints[1];
				b = parser.pints[2];
				 ...
				 * define VERTEX_FLAGS_OVERRIDE_MASK 0xf000 // this masks these bits to extract
				 * them ;
...
void Draw_RENDERLIST4DV2_Wire16(RENDERLIST4DV2_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
	...
	for(int poly = 0; poly < rend_list->num_polys; poly++)
	{
		...
		if
		(
			!(rend_list->poly_ptrs[poly]->state & POLY4DV2_STATE_ACTIVE)
...
void Draw_RENDERLIST4DV2_Wire(RENDERLIST4DV2_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
...
void Draw_RENDERLIST4DV2_Solid16(RENDERLIST4DV2_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
...
void Draw_RENDERLIST4DV2_Solid(RENDERLIST4DV2_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
...
void Draw_OBJECT4DV2_Solid(OBJECT4DV2_PTR obj, UCHAR *video_buffer, int lpitch)
{
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		if(obj->plist[poly].attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE)
		{
			...
			(
				(obj->plist[poly].attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
...
void Draw_OBJECT4DV2_Solid16(OBJECT4DV2_PTR obj, UCHAR *video_buffer, int lpitch)
{
...
(
	POLYF4DV2_PTR	face,			/* ptr to face */
...
)	/* bytes per line, 320, 640 etc. */
{
	...
	u, v, du, dv, xi, yi,		/* the current interpolated x,y */
	ui, vi, /* the current interpolated u,v */
	index_x, index_y,	/* looping vars */
	x, y,	/* hold general x,y */
	xstart, xend, ystart, yrestart, yend, xl, dxdyl, xr, dxdyr, dudyl, ul, dvdyl, vl, dudyr, ur, dvdyr, vr;
	int		x0, y0, tu0, tv0,	/* cached vertices */
	x1, y1, tu1, tv1, x2, y2, tu2, tv2;
	UCHAR	*screen_ptr = NULL, *screen_line = NULL, *textmap = NULL;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	&&	(face->tvlist[2].y < min_clip_y)
		...
		&&	(face->tvlist[2].y > max_clip_y)
			...
			&&	(face->tvlist[2].x < min_clip_x)
				...
				||	((face->tvlist[0].x > max_clip_x) && (face->tvlist[1].x > max_clip_x) && (face->tvlist[2].x > max_clip_x))
	) return;

	/* degenerate triangle */
	if
	(
		((face->tvlist[0].x == face->tvlist[1].x) && (face->tvlist[1].x == face->tvlist[2].x))
	...
	if(tri_type & TRI_TYPE_FLAT_MASK)
	{
		...
		if((yend = y2) > max_clip_y) yend = max_clip_y;
		...
		)
		{
			...
			for(yi = ystart; yi <= yend; yi++)
			{
				...
				if(xend > max_clip_x) xend = max_clip_x;
...
)	/* bytes per line, 320, 640 etc. */
{
	...
	UCHAR	*screen_ptr = NULL, *screen_line = NULL, *textmap = NULL, *lightrow444_8 = NULL;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
)	/* bytes per line, 320, 640 etc. */
{
	...
	int		x0, y0, tu0, tv0,	/* cached vertices */
	x1, y1, tu1, tv1, x2, y2, tu2, tv2;
	USHORT	*screen_ptr = NULL, *screen_line = NULL, *textmap = NULL, *dest_buffer = (USHORT *) _dest_buffer;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	else if(tri_type == TRI_TYPE_GENERAL)
	{
		...
		else
		{
			...
			/*
			 * no x clipping ;
			 * point screen ptr to starting lin
			 */
			screen_ptr = dest_buffer + (ystart * mem_pitch);
...
)	/* bytes per line, 320, 640 etc. */
{
	...
	x, y,	/* hold general x,y */
	xstart, xend, ystart, yrestart, yend, xl, dxdyl, xr, dxdyr, dudyl, ul, dvdyl, vl, dudyr, ur, dvdyr, vr;
	USHORT	r_base, g_base, b_base, r_textel, g_textel, b_textel, textel;
	int		x0, y0, tu0, tv0,	/* cached vertices */
	x1, y1, tu1, tv1, x2, y2, tu2, tv2;
	USHORT	*screen_ptr = NULL, *screen_line = NULL, *textmap = NULL, *dest_buffer = (USHORT *) _dest_buffer;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
		...
		)
		{
			...
			for(yi = ystart; yi <= yend; yi++)
			{
				...
				for(xi = xstart; xi <= xend; xi++)
				{
					...
					/*
					 * finally write pixel, note that we did the math such that the results are r*32,
					 * g*64, b*32 ;
					 * hence we need to divide the results by 32,64,32 respetively, BUT since we need
					 * to shift ;
					 * the results to fit into the destination 5.6.5 word, we can take advantage of
					 * the shifts ;
					 * and they all cancel out for the most part, but we will need logical anding, we
					 * will do ;
...
)
{
	...
	if(y3 > max_clip_y) y3 = max_clip_y;
...
)
{
	 ...
	 * it decomposes all triangles into a pair of flat top, flat botto
	 */
	...
	if((FCMP(x1, x2) && FCMP(x2, x3)) || (FCMP(y1, y2) && FCMP(y2, y3))) return;
...
int Load_Bitmap_PCX_File(BITMAP_FILE_PTR bitmap, char *filename)
{
	 ...
	 * decompress it and 3. load the palette dat
	 */
...
int Compute_OBJECT4DV2_Poly_Normals(OBJECT4DV2_PTR obj)
{
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		/*
		 * we need to compute the normal of this polygon face, and recall ;
		 * that the vertices are in cw order, u=p0->p1, v=p0->p2, n=ux
		 */
		VECTOR4D	u, v, n;
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
int Compute_OBJECT4DV2_Vertex_Normals(OBJECT4DV2_PTR obj)
{
	 ...
	 * to average the accumulated values, so instead of an O(n^2) algorithm, we get a
	 * O(c*n) ;
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		if(obj->plist[poly].attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
		{
			 ...
			 * to weight on the average, and since the length is in fact the area of the
			 * parallelogram ;
			 * constructed by uxv, so we are taking the "influence" of the area into
			 * consideration
			 */
	...
	/*
	 * now we are almost done, we have accumulated all the vertex normals, but need to
	 * average the
	 */
	for(int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		/*
		 * if this vertex has any contributors then it must need averaging, OR we could
		 * check ;
		 * the shading hints flags, they should be one to one
		 */
		Write_Error
		(
			"\nProcessing vertex: %d, attr: %d, contributors: %d",
			vertex,
...
)	/* bytes per line, 320, 640 etc. */
{
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*
	 * this function draws a gouraud shaded polygon, based on the affine texture
	 * mapper, instead ;
	 * of interpolating the texture coordinates, we simply interpolate the (R,G,B)
	 * values across ;
	 * the polygons, I simply needed at another interpolant, I have mapped u->red,
	 * v->green, w->blu
	 */
	int		v0 = 0, v1 = 1, v2 = 2, temp = 0, tri_type = TRI_TYPE_NONE, irestart = INTERP_LHS;
	int		dx, dy, dyl, dyr,		/* general deltas */
	u, v, w, du, dv, dw, xi, yi,	/* the current interpolated x,y */
	ui, vi, wi,			/* the current interpolated u,v */
	index_x, index_y,	/* looping vars */
	x, y,	/* hold general x,y */
	xstart, xend, ystart, yrestart, yend, xl, dxdyl, xr, dxdyr, dudyl, ul, dvdyl, vl, dwdyl, wl, dudyr, ur, dvdyr, vr, dwdyr, wr;
	int		x0, y0, tu0, tv0, tw0,	/* cached vertices */
	x1, y1, tu1, tv1, tw1, x2, y2, tu2, tv2, tw2;
	int		r_base0, g_base0, b_base0, r_base1, g_base1, b_base1, r_base2, g_base2, b_base2;
	USHORT	*screen_ptr = NULL, *screen_line = NULL, *textmap = NULL, *dest_buffer = (USHORT *) _dest_buffer;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	 ...
	 * two hard coded versions, if we want support for both 5.6.5, and 5.5.
	 */
...
)	/* bytes per line, 320, 640 etc. */
{
	 ...
	 * the polygons, I simply needed at another interpolant, I have mapped u->red,
	 * v->green, w->blue ;
	 * note that this is the 8-bit version, and I have decided to throw caution at the
	 * wind and see ;
	 * what happens if we do a full RGB interpolation and then at the last minute use
	 * the color lookup ;
		...
		)
		{
			...
			for(yi = ystart; yi <= yend; yi++)
			{
				...
				for(xi = xstart; xi <= xend; xi++)
				{
					...
					/* write textel assume 5.6.5 */
					screen_ptr[xi] = rgblookup[
						(
							((ui >> (FIXP16_SHIFT + 3)) << 11) + ((vi >> (FIXP16_SHIFT + 2)) << 5) +
								(wi >> (FIXP16_SHIFT + 3))
						)];
...
int RGB_12_8_Lighting_Table_Builder(LPPALETTEENTRY src_palette, /* source palette */ UCHAR rgblookup[4096][256])	/* lookup
																													 * table
																													 * */
{
	 ...
	 * match for the given color modulation, however, that would be 256*65536 = 16.7
	 * megs! ;
	 ...
	 * but not the light color, thus, the cache coherence will be excellent if we
	 * access ;
	 ...
	 */
	if(!src_palette || !rgblookup) return(-1);

	/*
	 * there are 4096 RGB values we need to compute, assuming that we are in RGB:
	 * 4.4.4 format
	 */
...
/*
 ...
 =======================================================================================================================
 */
void Draw_RENDERLIST4DV1_Solid2_16(RENDERLIST4DV1_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
...
void Draw_OBJECT4DV1_Solid2(OBJECT4DV1_PTR obj, UCHAR *video_buffer, int lpitch)
{
...
void Draw_RENDERLIST4DV1_Solid2(RENDERLIST4DV1_PTR rend_list, UCHAR *video_buffer, int lpitch)
{
...
void Draw_OBJECT4DV1_Solid2_16(OBJECT4DV1_PTR obj, UCHAR *video_buffer, int lpitch)
{
...
void Draw_RENDERLIST4DV2_Textured
(
...
void Draw_RENDERLIST4DV2_Textured16
(
	RENDERLIST4DV2_PTR	rend_list,
	UCHAR				*video_buffer,
	int					lpitch,
...
void Draw_RENDERLIST4DV1_Textured
(
...
void Draw_RENDERLIST4DV1_Textured16
(
	RENDERLIST4DV1_PTR	rend_list,
	UCHAR				*video_buffer,
	int					lpitch,
