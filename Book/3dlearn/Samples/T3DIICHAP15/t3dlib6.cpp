/*$T indentinput.cpp GC 1.140 12/29/10 17:26:16 */
...
void Draw_OBJECT4DV1_Solid16(OBJECT4DV1_PTR obj, UCHAR *video_buffer, int lpitch)
{
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		 ...
		 * render this polygon if and only if it's not clipped, not culled, ;
		 * active, and visible, note however the concecpt of "backface" is ;
		 * irrelevant in a wire frame engine thoug
		 */
		if
		(
			!(obj->plist[poly].state & POLY4DV1_STATE_ACTIVE)
...
/*
 =======================================================================================================================
    CLASS CPARSERV1 IMPLEMENTATION constructor
 =======================================================================================================================
 */
CPARSERV1::CPARSERV1()
{
	...
	if(fstream)
	{
		...
		if(mode & PARSER_STRIP_EMPTY_LINES)
		{
			...
			while(1)
			{
				...
				/* eat up space */
				while(isspace(string[sindex])) sindex++;
				...
				if((slength - sindex) > 0)
				{
					...
					if(mode & PARSER_STRIP_COMMENTS)
					{
						...
						if(comment_string == NULL) break;

						/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
						/* line is valid exit with line */
						...
						if(cindex == 0) continue;	/* this line is a comment, ignore completely, get another */
						else
						{
...
int CPARSERV1::Pattern_Match(char *string, char *pattern, ...)
{
	...
	if((!string || strlen(string) == 0) || (!pattern || strlen(pattern) == 0)) return(0);

	/* copy line into working area */
	strcpy(buffer, string);

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	int tok_start = 0, tok_end = 0, tok_restart = 0, tok_first_pass = 0, num_tokens = 0;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	while(1)
	{
		...
		/* eat whitepace */
		while(isspace(pattern[tok_start])) tok_start++;

		/* end of line? */
		if(tok_start >= strlen(pattern)) break;
			...
			case PATTERN_TOKEN_FLOAT:			/* float */
				{
					...
					if(pattern[tok_start + 2] != ']') return(0);	/* error */
			...
			case PATTERN_TOKEN_LITERAL: /* literal string */
				{
					...
					/* eat up string */
					while(pattern[tok_end] != PATTERN_TOKEN_LITERAL) tok_end++;

					/* make sure string is well formed */
					if(pattern[tok_end + 1] != ']') return(0);
			...
			case PATTERN_TOKEN_STRING:	/* ascii string varying length */
				{
					...
					if(pattern[tok_start + 2] == '=' || pattern[tok_start + 2] == '>' || pattern[tok_start + 2] == '<')
					{
						...
						while(isdigit(pattern[tok_end])) tok_end++;

						/* check for well formed */
						if(pattern[tok_end] != ']') return(0);
						...
						strcpy(token_string[num_tokens], "");					/* for literal strings this holds them */
						token_operator[num_tokens] = pattern[tok_start + 2];	/* holds and operators for the token,
																				 * >, <, =, etc. */
					...
					(
						...
						token_operator[num_tokens],
						token_numeric[num_tokens]
					);
#endif
		...
		case PATTERN_STATE_RESTART:
			{
				...
				/* error detection */
				if(tok_end >= strlen(buffer)) return(0);
		...
		case PATTERN_STATE_NEXT:
			{
				...
				else
				{
					...
					{
					case PATTERN_TOKEN_FLOAT:	{ pattern_state = PATTERN_STATE_FLOAT; }break;
					case PATTERN_TOKEN_INT:		{ pattern_state = PATTERN_STATE_INT; }break;
					case PATTERN_TOKEN_STRING:	{ pattern_state = PATTERN_STATE_STRING; }break;
					case PATTERN_TOKEN_LITERAL: { pattern_state = PATTERN_STATE_LITERAL; }break;
					default:					break;
					}		/* end switch */
				}			/* end else */
			}
...
int ReplaceChars(char *string_in, char *string_out, char *replace_chars, char rep_char, int case_on)
{
	...
	if(!string_in || !string_out || strlen(string_in) == 0) return(0);
	...
	if(case_on == 1)
	{
		...
		while(string_in[index_in])
		{
			...
			if(sindex >= slength) string_out[index_out++] = string_in[index_in++];
		}			/* end while */
	}	/* end if case_on */
	else
	{
...
int StripChars(char *string_in, char *string_out, char *strip_chars, int case_on)
{
	 ...
	 * if case_on = 1 then case is checked, other it's case insensitiv
	 */
...
char *StringRtrim(char *string)
{
	...
	while(isspace(string[sindex]) && sindex >= 0) string[sindex--] = 0;
...
char *StringLtrim(char *string)
{
	...
	while(isspace(string[sindex]) && sindex < slength) string[sindex++] = 0;	/* not needed actually */
...
int Convert_Bitmap_8_16(BITMAP_FILE_PTR bitmap)
{
	...
	for(int pixel = 0; pixel < bwidth * bheight; pixel++)
	{
		...
		buffer16[pixel] = RGB16Bit
			(
				bitmap->palette[cindex].peRed,
...
(
	OBJECT4DV1_PTR	obj,		/* pointer to object */
...
/* and perform transforms */
{
	 ...
	 * works with a OBJECT4DV1 which has no support for textures, or materials, etc,
	 * however we will ;
	...
	while(1)
	{
		...
		if(parser.Pattern_Match(parser.buffer, "['center'] [f] [f] [f]"))
		{
			 ...
			 * that we need to drop in each column of matri
			 */
...
int RGBto8BitIndex(UCHAR r, UCHAR g, UCHAR b, LPPALETTEENTRY palette, int flush_cache = 0)
{
	 ...
	 * the process, the last few translated colored are stored in a stack in the
	 * format ;
	 * rgbi, so when a new rgb comes in, it is compared against the rgb entries in the ;
	 * table, if found then that index is used, else, the rgb is translated and added
	 * to ;
	 * the table, and the table is shifted one slot, so the last element is thrown
	 * away, ;
	 * hence the table is FIFO in as much as the first discarded value will be the
	 * first ;
	 * this way the system keeps previously translated colors cached, so the fairly
	 * long ;
	 * least squared scan doesn't take forever! ;
	 * also note the compression of the RGBI data, a compare is performed on the upper
	 * 24 bits only ;
	 * also, if flush_cache = 1 then the local cache is flushed, for example a new
	 * palette is loaded
	 */
...
#define COLOR_CACHE_SIZE	16	/* 16 entries should do for now */
	...
	if(flush_cache == 1) cache_entries = 0;
	...
	for(int cache_index = 0; cache_index < cache_entries; cache_index++)
	{
		...
		/* is this a match? */
		if(r == color_cache[cache_index].r && g == color_cache[cache_index].g && b == color_cache[cache_index].b)
	...
	if(++cache_entries > COLOR_CACHE_SIZE) cache_entries = COLOR_CACHE_SIZE;
...
)	/* power factor/falloff for spot lights */
{
	...
	if(_pos) VECTOR4D_COPY(&lights[index].pos, _pos);	/* position of light */
...
(
	int								rgb_format,		/* format we want to build table for */
	/* 99/100 565 */LPPALETTEENTRY	src_palette,	/* source palette */
	UCHAR							*rgblookup
)	/* lookup table */
{

	/*
	 * this function takes as input the rgb format that it should generate the lookup
	 * table ;
	 * for;
	 * dd_pixel_format = DD_PIXEL_FORMAT565, or DD_PIXEL_FORMAT555 ;
	 ...
	 * in either case, it's up to the caller to send in the rgblookup table
	 * pre-allocated ;
	 * the function doesn't allocate memory for the caller ;
	 * the function uses a simple least squares scan for all possible RGB colors in
	 * the ;
	...
	if(rgb_format == DD_PIXEL_FORMAT565)
	{
		...
		for(int rgbindex = 0; rgbindex < 65536; rgbindex++)
		{
			...
			for(int color_index = 0; color_index < 256; color_index++)
			{
				...
				/*~~~~~~~~~~~~~~~~~~~~~~~~~~*/
				/*
				 * extract r,g,b from rgbindex, assuming an encoding of 5.6.5, then scale to 8.8.8
				 * since ;
...
)
{
	...
	/*~~~~~~~~~~~~~~~~~~~*/
	/*
	 * this function takes the source palette to compute the intensity shading table
	 * with ;
	 * the table will be formatted such that each row is a color index, and each
	 * column ;
	 * is the shade 0..255 desired, the output is a single byte index ;
	 * in either case, it's up to the caller to send in the rgbilookup table
	 * pre-allocated ;
	 * 64k buffer byte [256][256]the function doesn't allocate memory for the caller ;
	 * the function builds the table by looping thru each color in the color palette
	 * and then ;
	 * for each color, it scales the color to maximum intensity without overflow the
	 * RGB channels ;
	 * and then uses this as the 100% intensity value of the color, then the algorithm
	 * computes ;
	 * the 256 shades of the color, and then uses the standard least squares scan the
	 * find the ;
	 * colors in the palette and stores them in the row of the current color under
	 * intensity ;
	 * translation, sounds diabolical huh? Note: if you set intensity normalization to
	 * 0 ;
	...
	/* first check the pointers */
	if(!src_palette || !rgbilookup) return(-1);
	...
	for(int col_index = 0; col_index < 256; col_index++)
	{
		 ...
		 */
		dl = sqrt(double(ri * ri + gi * gi + bi * bi)) / (float) 256;
		dr = ri / dl, db = gi / dl, dg = bi / dl;
		...
		/*
		 * at this point rw,gw,bw, is the color that we need to compute the 256
		 * intensities for to ;
		...
		for(int intensity_index = 0; intensity_index < 256; intensity_index++)
		{
			...
			/*
			 * compute next intensity level (test for overflow, shouldn't happen, but never
			 * know)
			 */
			if(rw += dr > 255) rw = 255;
			if(gw += dg > 255) gw = 255;
...
int Insert_OBJECT4DV1_RENDERLIST4DV12
(
...
)
{
	 ...
	 * polygon when its sent to the rendering lis
	 */
...
(
	OBJECT4DV1_PTR	obj,	/* object to process */
...
(
	RENDERLIST4DV1_PTR	rend_list,	/* list to process */
	CAM4DV1_PTR			cam,		/* camera position */
	LIGHTV1_PTR			lights,		/* light list (might have more than one) */
	int					max_lights
)	/* maximum lights in list */
{
	...
	/*
	 * 16-bit version of function ;
	 * function lights the enture rendering list based on the sent lights and camera.
		...
		if(curr_poly->attr & POLY4DV1_ATTR_SHADE_MODE_FLAT || curr_poly->attr & POLY4DV1_ATTR_SHADE_MODE_GOURAUD)
		{
			...
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
			{
				...
				else if(lights[curr_light].attr & LIGHTV1_ATTR_INFINITE)
				{
					 ...
					 * notice the scaling by 128, I want to avoid floating point calculations, not
					 * because they ;
					 * are slower, but the conversion to and from cost cycle
					 */
					dp = VECTOR4D_Dot(&n, &lights[curr_light].dir);
