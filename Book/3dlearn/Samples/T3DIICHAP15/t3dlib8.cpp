/*$T indentinput.cpp GC 1.140 12/29/10 17:26:20 */
...
void Clip_Polys_RENDERLIST4DV2(RENDERLIST4DV2_PTR rend_list, CAM4DV1_PTR cam, int clip_flags)
{
	...
	/*
	* this function clips the polygons in the list against the requested clipping
	* planes ;
	...
	* be culled, however, this test isn't as effective on games based on objects
	* since ;
	* in most cases objects that are visible have polygons that are visible, but in
	* the ;
	* case where the polygon list is based on a large object that ALWAYS has some
	* portion ;
			...
			if(((vertex_ccodes[0] | vertex_ccodes[1] | vertex_ccodes[2]) & CLIP_CODE_LZ))
			{
				...
				*/
				...
				{		/* num_verts = 2
					...
					* the split ;
					* now overwrite vertex with new verte
					*/
 ...
 )	/* the shading attributes we would like */
{
	...
	* the call parameters are redundant in this case, but who care
	*/
	...
	*/
	...
	Write_Error("\nObject average radius = %f, max radius = %f", obj->avg_radius[0], obj->max_radius[0]);
	...
	for(int poly = 0; poly < obj->num_polys / 2; poly++)
	{
		...
		*/
		...
		if
			(
			(obj->plist[poly * 2].attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
		...
		* whether the local or transformed vertex list is used when building up ;
		* polygon geometry, might be a better idea to set to NULL in the context ;
		* of polygons that are part of an objec
		*/
		obj->plist[poly * 2].vlist = obj->vlist_local;
		obj->plist[poly * 2 + 1].vlist = obj->vlist_local;
...
(
 LIGHTV2_PTR		lights,			/* light array to work with (new) */
 ...
 )	/* power factor/falloff for spot lights */
{
	...
	* aren't needed are set to 0 by caller, nearly identical to version 1.0, however
	* has ;
	* new support for transformed light coordinates ;
	* make sure light is in rang
	*/
...
int Reset_Lights_LIGHTV2(LIGHTV2_PTR lights, /* light array to work with (new) */ int max_lights)	/* number of lights
																									* in system */
{
		...
		if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
		{
			...
			*/
			...
			for(int curr_light = 0; curr_light < max_lights; curr_light++)
			{
				...
				if(lights[curr_light].state == LIGHTV2_STATE_OFF) continue;

				/*
				* Write_Error("\nprocessing light %d",curr_light);
				* * what kind of light are we dealing wit
				*/
				...
				else if(lights[curr_light].attr & LIGHTV2_ATTR_INFINITE)
				{
				...
				else if(lights[curr_light].attr & LIGHTV2_ATTR_POINT)
				{
				...
				else if(lights[curr_light].attr & LIGHTV2_ATTR_SPOTLIGHT1)
				{
				...
				else if(lights[curr_light].attr & LIGHTV2_ATTR_SPOTLIGHT2)	/* simple version */
				{
	...
	for(int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		*/
		if(curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
		{
			...
			* Write_Error("\nWriting final values to polygon %d = %d,%d,%d", poly, r_sum,
			* g_sum, b_sum);
			* * write the color over current colo
			*/
