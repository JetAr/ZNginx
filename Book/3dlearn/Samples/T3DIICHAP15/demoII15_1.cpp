/*$T indentinput.cpp GC 1.140 12/29/10 17:24:47 */
...
/* DEFINES defines for windows interface */
...
gray, black, red, green, blue, yellow, orange;

ZBUFFERV1		zbuffer;		/* our little z buffer! */
RENDERCONTEXTV1 rc;			/* the rendering
							 * context;
							 * */
...
int Game_Init(void *parms)
{
	...
	/* hide the mouse */
	if(!WINDOWED_APP) ShowCursor(FALSE);
	...
	{
		Load_OBJECT4DV2_COB2
		(
			&obj_light_array[index_obj],
			object_light_filenames[index_obj],
			&vscale,
			&vpos,
			&vrot,
			VERTEX_FLAGS_INVERT_WINDING_ORDER | VERTEX_FLAGS_TRANSFORM_LOCAL | VERTEX_FLAGS_TRANSFORM_LOCAL_WORLD,
			0
		);
	}	/* end for index */
	...
	(
		...
		gray,
		black,
		black,	/* color for ambient term only */
		NULL,
		NULL,	/* no need for pos or dir */
		0,
		0,
		0,		/* no need for attenuation */
		0,
		0,
		0
	);
	...
	(
		...
		black,
		gray,
		black,			/* color for diffuse term only */
		NULL,
		&dlight_dir,	/* need direction only */
		0,
		0,
		0,	/* no need for attenuation */
		0,
	...
	(
		DD_PIXEL_FORMAT565, /* format we want to build table for */
	...
	Load_Object_MD2
	(
		&obj_md2,	/* the loaded md2 file placed in container */
		"./md2/q2mdl-tekkblade/tris.md2",			/* "D:/Games/quakeII/baseq2/players/male/tris.md2", // the filename
													 * of the .MD2 model */
		&vs,
		&vp,
		NULL,
		"./md2/q2mdl-tekkblade/blade_black.bmp",	/* "D:/Games/quakeII/baseq2/players/male/claymore.bmp", // the
													 * texture filename for the model */
	...
	Prepare_OBJECT4DV2_For_MD2(&obj_model, /* pointer to destination object */ &obj_md2);	/* md2 object to extract
																							 * frame from */
...
int Game_Main(void *parms)
{
	...
	/*
	 * draw the ground ;
	 * Draw_Rectangle(0,WINDOW_HEIGHT*.38, WINDOW_WIDTH, WINDOW_HEIGHT,
	 * RGB16Bit(25,50,110), lpddsback);
	 * ;
	...
	{
		if(++obj_md2.anim_state >= NUM_MD2_ANIMATIONS) obj_md2.anim_state = 0;
	...
	{
		if(--obj_md2.anim_state < 0) obj_md2.anim_state = NUM_MD2_ANIMATIONS - 1;
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*
	 * motion section terrain following, simply find the current cell we are over and
	 * then ;
	...
	if((plight_ang += 1) > 360) plight_ang = 0;
	...
	/*
	 * render model, this next section draws each copy of the mech model animate the
	 * model
	 */
	Animate_MD2(&obj_md2);

	/* extract the frame of animation from vertex banks */
	Extract_MD2_Frame(&obj_model, /* pointer to destination object */ &obj_md2);	/* md2 object to extract frame from */
	 ...
	 * remove backface
	 */
	if(backface_mode == 1) Remove_Backfaces_RENDERLIST4DV2(&rend_list, &cam);
	...
	if(zsort_mode == 1) Sort_RENDERLIST4DV2(&rend_list, SORT_POLYLIST_AVGZ);
	...
	for(; pindex < obj_model.num_polys; pindex++)
	{
		...
		obj_model.plist[pindex].attr = POLY4DV2_ATTR_RGB16 |
	...
	for(; vertex < obj_model.num_vertices; vertex++)
	{
		/*~~~~~~~~~~~~~~~~~~*/
		...
		obj_model.vlist_trans[vertex].v.y = 10.0;	/* pl.y + t0*(vi.y
													 * pl.y);
													 * */
	...
	else if(wireframe_mode == 1)
	{
		...
		/*
		 * perspective mode affine texturing ;
		 * set up rendering contex
		 */
		rc.attr = RENDER_ATTR_ZBUFFER | RENDER_ATTR_ALPHA
	...
	if(help_mode == 1)
	{
		...
		Draw_Text_GDI
		(
			"<3>,<4>..........Play Animation Single Shot/Looped.",
			0,
			text_y += 12,
			RGB(255, 255, 255),
			lpddsback
	...
	(
		work_string,
		"Lighting [%s]: Ambient=%d, Infinite=%d, Point=%d, BckFceRM [%s], Zsort[%s]",
	...
	(
		work_string,
		"Polys Rendered: %d, Polys lit: %d Anim[%d]=%s Frm=%d",
		debug_polys_rendered_per_frame,
		debug_polys_lit_per_frame,
		obj_md2.anim_state,
		md2_anim_strings[obj_md2.anim_state],
		obj_md2.curr_frame
	);
