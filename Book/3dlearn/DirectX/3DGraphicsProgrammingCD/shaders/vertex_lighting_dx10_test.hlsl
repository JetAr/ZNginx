...
float4 g_vGlobal_AmbientLight = 0.1f;
...
VS_OUTPUT VS(VS_INPUT In)
{
	...
	for ( int i=0; i<3; i++ )
	{
		int type = g_Lights[i].m_iLightInfo.x;
		int enabled = g_Lights[i].m_iLightInfo.y;
		...
		if ( enabled )
		{
			...
			case 3: // spot light
				{
					float  m_fSpotlightInnerCone = g_Lights[i].m_vSpotLightCoeff.x;
					float  m_fSpotlightCutoffCosine = g_Lights[i].m_vSpotLightCoeff.y;
					float  m_fSpotlightExponent = g_Lights[i].m_vSpotLightCoeff.z;	
					...
					if ( fCosine >= m_fSpotlightCutoffCosine )
					{
						float  SpotlightEffect = pow(fCosine, m_fSpotlightExponent);
