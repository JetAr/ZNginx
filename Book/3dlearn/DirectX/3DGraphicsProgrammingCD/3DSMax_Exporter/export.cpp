...
void GmodelExp::ExportGlobalInfo()
{
...
void GmodelExp::ExportGeomObject(INode* node, int indentLevel)
{
...
void GmodelExp::ExportNodeHeader(INode* node, TCHAR* type, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
void GmodelExp::ExportNodeTM(INode* node, int indentLevel)
{
...
void GmodelExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
	int i;
...
Point3 GmodelExp::GetVertexNormal(Mesh* mesh, int faceNo, RVertex* rv)
{
...
void GmodelExp::ExportMaterialList()
{
	if (!GetIncludeMtl()) {
		return;
	}
...
void GmodelExp::ExportMaterial(INode* node, int indentLevel)
{
	Mtl* mtl = node->GetMtl();
...
void GmodelExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo, int indentLevel)
{
	int i;
...
TCHAR* GmodelExp::GetMapID(Class_ID cid, int subNo)
{
	static TCHAR buf[50];
...
void GmodelExp::DumpTexture(Texmap* tex, Class_ID cid, int subNo, float amt, int indentLevel)
{
	if (!tex) return;
...
void GmodelExp::DumpUVGen(StdUVGen* uvGen, int indentLevel)
{
...
BOOL GmodelExp::CheckForAndExportFaceMap(Mtl* mtl, Mesh* mesh, int indentLevel)
{
	if (!mtl || !mesh) {
		return FALSE;
	}
...
TSTR GmodelExp::GetIndent(int indentLevel)
{
	TSTR indentString = "";
...
BOOL GmodelExp::TMNegParity(Matrix3 &m)
{
...
TriObject* GmodelExp::GetTriObjectFromNode(INode *node, TimeValue t, int &deleteIt)
{
	deleteIt = FALSE;
...
void GmodelExp::DumpMatrix3(Matrix3* m, int indentLevel)
{
	Point3 row;
	TSTR indent = GetIndent(indentLevel);
...
void GmodelExp::make_face_uv(Face *f, Point3 *tv)
{
	int na,nhid,i;
	Point3 *basetv;
...
TCHAR* GmodelExp::FixupName(TCHAR* name)
{
...
// format consistent.
void GmodelExp::CommaScan(TCHAR* buf)
{
	for(; *buf; buf++) if (*buf == ',') *buf = '.';
}

TSTR GmodelExp::Format(int value)
{
	TCHAR buf[50];
...
TSTR GmodelExp::Format(float value)
{
	TCHAR buf[40];
...
TSTR GmodelExp::Format(Point3 value)
{
	TCHAR buf[120];
	TCHAR fmt[120];
...
TSTR GmodelExp::Format(Color value)
{
	TCHAR buf[120];
	TCHAR fmt[120];
...
TSTR GmodelExp::Format(AngAxis value)
{
	TCHAR buf[160];
	TCHAR fmt[160];
...
TSTR GmodelExp::Format(Quat value)
{
...
TSTR GmodelExp::Format(ScaleValue value)
{
	TCHAR buf[280];
