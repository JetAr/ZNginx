/*	MultiSemx.c  DEFECTIVE VERSION
	...
	4.	Mutexs are used for critical sections. These could be replaced with
		...
		capabilities. Also, you will loose the opportunity to use
		SignalObjectAndWait.
...
/*********************************************
	...
	higher priority. *** Be very careful as a large request may block
	(starve) all other threads. Do not wait for a large count, especially
	in FCFS mode, if you can not assure that the count will be satisifed
	eventually.
*/
...
/*	Multiple wait semaphore creation.
	...
	functions will, in all liklihood, result in a defect):
		1.	No thread can set, pulse, or reset the event,
			...
			This is a consequence of using a "signal" condition variable model.
