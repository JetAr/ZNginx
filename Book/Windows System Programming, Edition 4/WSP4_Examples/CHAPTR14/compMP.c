...
int _tmain (int argc, LPTSTR argv[])
{
    ...
    for (i = 0; i < nThreads; i++)
    {
        ...
        pThArg->pFile1 = pFile1;
        pThArg->pFile2 = pFile2;
    ...
    free(threadArgs);
    UnmapViewOfFile (pFile1);
    UnmapViewOfFile (pFile2);
    CloseHandle (hFile1);
    CloseHandle (hFile2);
...
DWORD WINAPI ReadCompare (LPVOID pArg)
{
    ...
    while (curPos < fileSize)
    {
        ...
        nBytes = (size_t)min(blockSize, (fileSize - curPos));
        nQuads = nBytes / unitSize;
        nResidual = nBytes % unitSize;
...
void LogMismatchByte(unsigned __int64 curPos, PBYTE pF1Start, PBYTE pb1, PBYTE pb2)
{
    ...
    __try
    {
        ...
        while (insertHere < MAX_MISMATCH && mmLoc > misMatch.mmLocation[insertHere])
        {
            insertHere++;
        }
        ...
        misMatch.byte1[insertHere] = *pb1;
        misMatch.byte2[insertHere] = *pb2;
...
int ConventionalFileAccess (HANDLE hFile1, HANDLE hFile2, unsigned int blockSize, unsigned __int64 fileSize)
{
    ...
    llBuffer1 = (__int64 *)bBuffer1;
    llBuffer2 = (__int64 *)bBuffer2;
