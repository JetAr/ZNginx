...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 7.1", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
    ...
    if(!m_deviceLost)
    {
        try
        {
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 5.0f, -20.0f), &D3DXVECTOR3(0.0f, 2.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
