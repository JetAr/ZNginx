        ...
        LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
    ...
    if(pSkinInfo != NULL)
    {
        ...
        //Find out how the vertex data is done
        D3DVERTEXELEMENT9 d[MAX_FVF_DECL_SIZE];
        boneMesh->MeshData.pMesh->GetDeclaration(d);

        for(int i=0; i<MAX_FVF_DECL_SIZE; i++)
        {
            if(d[i].Type == D3DDECLTYPE_UNUSED)
            {
                break;
            }

            g_debug << "Vertex Element, Type: " << (int)d[i].Type << ", Usage: " << (int)d[i].Usage << ", Offset: " << (int)d[i].Offset << "\n";
        }
