...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 8.1", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
    ...
    if(!m_deviceLost)
    {
        try
        {
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                RECT r = {10, 10, 800, 600};
                g_pFont->DrawText(NULL, "KeyUp / KeyDown, Change Blend Amount:", -1, &r, DT_TOP | DT_LEFT | DT_NOCLIP, 0x88000000);

                char prc[10];
                _itoa((int)(m_morphTest.m_blend * 100), prc, 10);
                strcat(prc, "%");
                r.left += 380;
                g_pFont->DrawText(NULL, prc, -1, &r, DT_TOP | DT_LEFT | DT_NOCLIP, 0x88000000);
