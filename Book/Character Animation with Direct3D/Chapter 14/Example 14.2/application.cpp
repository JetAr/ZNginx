...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 14.2", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
...
void Application::Update(float deltaTime)
{
    try
    {
        ...
        //Add decal
        m_decalCooldown -= deltaTime;
        if(KeyDown(VK_LBUTTON) && m_decalCooldown <= 0.0f)
        {
            m_drone.AddDecal();
            m_decalCooldown= 0.3f;
        }
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        try
        {
            ...
            D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 1.2f, -3.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                RECT rc = {10, 10, 0, 0};
                g_pFont->DrawText(NULL, "Move the ray with the mouse.\nPress mouse button to add decal.", -1, &rc, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);
