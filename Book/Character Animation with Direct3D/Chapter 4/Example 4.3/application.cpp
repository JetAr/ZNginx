...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 4.3", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
    ...
    for(int i=0; i<4; i++)
    {
        ...
        m_animControllers.push_back(m_drone.GetController());
    }
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        try
        {
            ...
            D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 1.5f, -4.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                //Render Drone
                {
                    ...
                    int numSoldiers = (int)m_positions.size();

                    for(int i=0; i<numSoldiers; i++)
                    {
                        m_animControllers[i]->AdvanceTime(m_deltaTime, NULL);
                        m_drone.SetPose(m_positions[i]);
                        m_drone.Render(NULL);
                    }
                }
