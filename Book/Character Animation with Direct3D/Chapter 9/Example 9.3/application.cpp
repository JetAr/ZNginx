...
private:
    ...
    Face *m_pFace;
    vector<FaceController*> m_faceControllers;
    float m_time;
};
...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 9.3", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    ...
    m_faceControllers.push_back(new FaceController(D3DXVECTOR3(0.0f, 0.0f, 0.0f), m_pFace));
    m_faceControllers.push_back(new FaceController(D3DXVECTOR3(0.3f, 0.0f, 0.0f), m_pFace));
    m_faceControllers.push_back(new FaceController(D3DXVECTOR3(-0.3f, 0.0f, 0.0f), m_pFace));
...
void Application::Update(float deltaTime)
{
    try
    {
        ...
        for(int i=0; i<(int)m_faceControllers.size(); i++)
        {
            m_faceControllers[i]->Update(deltaTime);
        }
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        try
        {
            ...
            D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 0.0f, -1.2f), &D3DXVECTOR3(0.0f, 0.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
