...
private:
    ...
    FaceFactory *m_pFaceFactory;

    Face* m_face;
    FaceController* m_faceController;
...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 9.4", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    ...
    //Init the face factory
    m_pFaceFactory = new FaceFactory("resources/factory_face.x");
...
void Application::Update(float deltaTime)
{
    try
    {
        ...
        {
            m_faceController->Update(deltaTime);
        }
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        try
        {
            ...
            D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 0.0f, -0.75f), &D3DXVECTOR3(0.0f, 0.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
            D3DXMatrixPerspectiveFovLH(&proj, D3DX_PI / 6.0f, (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, 0.05f, 1000.0f);
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                if(m_faceController != NULL)
                {
                    m_faceController->Render();
                }

                RECT rc = {10, 10, 0, 0};
                g_pFont->DrawText(NULL, "Press Space to Generate a new Face", -1, &rc, DT_TOP | DT_LEFT | DT_NOCLIP, 0x88000000);
...
void Application::GenerateFace()
{
    ...
    {
        RECT rc = {0, 0, 800, 600};
        g_pFont->DrawText(NULL, "GENERATING FACE", -1, &rc, DT_VCENTER | DT_CENTER | DT_NOCLIP, 0xff000000);
    ...
    if(m_faceController != NULL)
    {
        delete m_faceController;
        m_faceController = NULL;
    }

    //Generate new face + controller
    m_face = m_pFaceFactory->GenerateRandomFace();
    m_faceController = new FaceController(D3DXVECTOR3( 0.0f, 0.0f, 0.0f), m_face);
}
