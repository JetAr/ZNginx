...
Face::Face(string filename)
{
    ...
    D3DXCreateTextureFromFile(g_pDevice, "resources/head_base.TSNM.dds", &m_pFaceNormalMap);
    D3DXCreateTextureFromFile(g_pDevice, "resources/blend_mask.tga", &m_pBlendMap);
}
...
void Face::Render(FaceController *pController)
{
    ...
    g_pEffect->SetTexture("texBlendMap", m_pBlendMap);
    g_pEffect->SetVector("weights", &pController->m_morphWeights);
    g_pEffect->SetFloat("foreheadWeight", pController->m_morphWeights.y);
    g_pEffect->SetFloat("cheekWeight", pController->m_morphWeights.z);
