...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 12.2", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    //SetCursor(NULL);
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        ...
        {
            //Create Matrices
            D3DXMATRIX identity, world, view, proj, worldInverse;
            D3DXVECTOR3 cameraPos(0.0f, 0.0f, -0.5f);

            D3DXMatrixIdentity(&identity);
            D3DXMatrixLookAtLH(&view, &cameraPos, &D3DXVECTOR3(0.0f, 0.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
            D3DXMatrixPerspectiveFovLH(&proj, D3DX_PI / 4.0f, (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, 0.1f, 10.0f);
            D3DXMatrixInverse(&worldInverse, NULL, &world);

            //Calculate Light pos
            D3DXVECTOR4 lightPos(cos(m_time) * 75, sin(m_time) * 15 + 10.0f, -75, 0.0f);
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                //Render Drone
                {
                    ...
                    g_pEffect->SetVector("cameraPos", &D3DXVECTOR4(cameraPos.x, cameraPos.y, cameraPos.z, 0.0f));
