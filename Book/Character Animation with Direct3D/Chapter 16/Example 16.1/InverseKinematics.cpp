...
InverseKinematics::InverseKinematics(SkinnedMesh* pSkinnedMesh)
{
    ...
    // Hand IK bones
    m_pShoulderBone = (Bone*)m_pSkinnedMesh->GetBone("Upper_Arm_Left");
    m_pElbowBone = (Bone*)m_pSkinnedMesh->GetBone("Lower_Arm_Left");
    m_pHandBone = (Bone*)m_pSkinnedMesh->GetBone("Hand_Left");
}
...
{
    // Calculate the angle between the forward vector for the vector from bone to lookAtTarget
    // This algorithm is also commonly called shortest arc rotation
    // I.e. first we need we find an axis that can rotate vector A to vector B
    // this axis is a vector that is perpendicular to both A and B, i.e. let's use the cross product.
    // The angle used to rotate from A to B can be calculated with the dot product.
