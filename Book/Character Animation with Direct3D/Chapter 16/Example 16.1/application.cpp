...
FaceFactory *g_pFaceFactory = NULL;;
...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 16.1", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
    ...
    //Init Face Factory
    g_pFaceFactory = new FaceFactory("resources/facefactory/factory_face.x");

    //Create character
    D3DXMATRIX world;
    D3DXMatrixIdentity(&world);
    m_pDrone = new Character("resources/meshes/soldier.x", world);
...
void Application::Update(float deltaTime)
{
    try
    {
        ...
        //Physics Engine
        physicsEngine.Update(deltaTime * 0.75f);

        //Update Character
        m_pDrone->Update(deltaTime);
    ...
    if(!m_deviceLost)
    {
        try
        {
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                D3DXMatrixIdentity(&world);
                D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0.0f, 2.0f, -2.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
                ...
                {
                    RECT rc1 = {10, 10, 0, 0};
                    g_pFont->DrawText(NULL, "Space: Randomize Face", -1, &rc1, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);

                    RECT rc2 = {10, 30, 0, 0};
                    g_pFont->DrawText(NULL, "Enter: Switch Animation", -1, &rc2, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);

                    RECT rc3 = {10, 50, 0, 0};
                    string s = "L: Look-At IK";
                    s += (m_pDrone->m_lookAtIK ? " (ON)" : " (OFF)");
                    g_pFont->DrawText(NULL, s.c_str(), -1, &rc3, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);

                    RECT rc4 = {10, 70, 0, 0};
                    s = "A: Arm IK";
                    s += (m_pDrone->m_armIK ? " (ON)" : " (OFF)");
                    g_pFont->DrawText(NULL, s.c_str(), -1, &rc4, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);

                    RECT rc5 = {10, 100, 0, 0};
                    g_pFont->DrawText(NULL, "K: KILL!", -1, &rc5, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);
                }
                else
                {
                    RECT rc1 = {10, 10, 0, 0};
                    g_pFont->DrawText(NULL, "Space: Revive Character", -1, &rc1, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);
                }
