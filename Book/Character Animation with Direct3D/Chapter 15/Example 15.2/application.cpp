...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 15.2", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
...
void Application::Render()
{
    if(!m_deviceLost)
    {
        try
        {
            //Create Matrices
            float radius = 0.6f;
            D3DXVECTOR3 camPos(cos(m_angle) * radius, 0.0f, sin(m_angle) * radius);

            D3DXMATRIX identity, world, view, proj;
            D3DXMatrixIdentity(&identity);
            D3DXMatrixLookAtLH(&view, &camPos, &D3DXVECTOR3(0.0f, 0.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
            ...
            if(SUCCEEDED(g_pDevice->BeginScene()))
            {
                ...
                m_pHair->Render(camPos);

                RECT rc = {10, 10, 0, 0};
                g_pFont->DrawText(NULL, "SPACE: Draw Debug Info\nClick n Drag to rotate view", -1, &rc, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);

                //Print Hair Stats
                string text = "Num Vertices: ";
                char temp[10];
                itoa(m_pHair->GetNumVertices(), temp, 10);
                text += temp;
                text += "\nNum Faces: ";
                itoa(m_pHair->GetNumFaces(), temp, 10);
                text += temp;

                RECT rc2 = {10, 560, 0, 0};
                g_pFont->DrawText(NULL, text.c_str(), -1, &rc2, DT_LEFT | DT_TOP | DT_NOCLIP, 0x88000000);
