...
#define DRAW_R				( VIEW_WIDTH + VIEW_HEIGHT )	// 渲染半径
...
int FlushDrawingPictures( void );				// 顺序输出等待渲染的图形矩阵
...
int DrawChangingPictures( void )						// 每帧调用
{
    ...
    xt = VIEW_WIDTH  + DRAW_R * cosf( 3.0f * PI / 2.0f - fAngle1 );
    yt = VIEW_HEIGHT + DRAW_R * sinf( 3.0f * PI / 2.0f - fAngle1 );
    Draw2DPolygon( VIEW_WIDTH, VIEW_HEIGHT, 1.0f, 1.0f,
                   xt, yt, xt / VIEW_WIDTH, yt / VIEW_HEIGHT,
                   -VIEW_HEIGHT, VIEW_HEIGHT, -( float )VIEW_HEIGHT / VIEW_WIDTH, 1.0f,
                   &g_tPic1 );
    Draw2DPolygon( VIEW_WIDTH, VIEW_HEIGHT, 1.0f, 1.0f,
                   VIEW_WIDTH, -VIEW_WIDTH, 1.0f, -( float )VIEW_WIDTH / VIEW_HEIGHT,
                   xt, yt, xt / VIEW_WIDTH, yt / VIEW_HEIGHT,
                   &g_tPic2 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Wipe_5_1" ),
