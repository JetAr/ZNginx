...
int InitBack( void )							// 只在程序开始时调用一次
{
    ...
    fBack_x = VIEW_WIDTH / 2.0f - fCamera_x;	// 背景的初始位置②
...
int MoveBack( void )							// 每帧调用
{
    ...
    fBack_x = VIEW_WIDTH / 2.0f - fCamera_x;	// 背景的位置⑤
...
HRESULT InitGeometry( void )
{
    ...
    hr = LoadTexture( _T( "7.dds" ), &g_tBack );//, 1600, 480, 2048, 512 );
    if ( FAILED( hr ) )
    {
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Scroll_1_1" ),
