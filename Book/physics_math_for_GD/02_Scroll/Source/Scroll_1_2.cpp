...
TEX_PICTURE				g_tBack1, g_tBack2, g_tBack3;
...
HRESULT InitGeometry( void )
{
    ...
    // 创建纹理
    g_tBack1.pSRViewTexture =  NULL;
    hr = LoadTexture( _T( "7.dds" ), &g_tBack1 );
    if ( FAILED( hr ) )
    {
        MessageBox( NULL, _T( "Can't open 7.dds" ), _T( "Error" ), MB_OK );
        return hr;
    }
    g_tBack2.pSRViewTexture =  NULL;
    hr = LoadTexture( _T( "6.dds" ), &g_tBack2 );
    if ( FAILED( hr ) )
    {
        MessageBox( NULL, _T( "Can't open 6.dds" ), _T( "Error" ), MB_OK );
        return hr;
    }
    g_tBack3.pSRViewTexture =  NULL;
    hr = LoadTexture( _T( "5.dds" ), &g_tBack3 );
    if ( FAILED( hr ) )
    {
        MessageBox( NULL, _T( "Can't open 5.dds" ), _T( "Error" ), MB_OK );
        return hr;
    }
...
int Cleanup( void )
{
    SAFE_RELEASE( g_tBack1.pSRViewTexture );
    SAFE_RELEASE( g_tBack2.pSRViewTexture );
    SAFE_RELEASE( g_tBack3.pSRViewTexture );
...
HRESULT Render( void )
{
    ...
    DrawPicture( fBack_x3, 0.0f, &g_tBack3 );
    g_pImmediateContext->OMSetBlendState( g_pbsAlphaBlend, NULL, 0xFFFFFFFF );
    DrawPicture( fBack_x2, 0.0f, &g_tBack2 );
    DrawPicture( fBack_x1, 0.0f, &g_tBack1 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Scroll_1_2" ),
