...
int MoveChara( void )							// 每帧调用
{
    ...
    fCamera_x = fChara_sx;										// 镜头暂时回到角色的位置
...
HRESULT InitGeometry( void )
{
    ...
    hr = LoadTexture( _T( "6.dds" ), &g_tBack );
    if ( FAILED( hr ) )
    {
        MessageBox( NULL, _T( "Can't open 6.dds" ), _T( "Error" ), MB_OK );
        return hr;
    }
    g_tChara.pSRViewTexture =  NULL;
    hr = LoadTexture( _T( "8_影あり.dds" ), &g_tChara );
    if ( FAILED( hr ) )
    {
        MessageBox( NULL, _T( "Can't open 8_影あり.dds" ), _T( "Error" ), MB_OK );
        return hr;
    }
...
int Cleanup( void )
{
    ...
    SAFE_RELEASE( g_tChara.pSRViewTexture );
...
HRESULT Render( void )
{
    ...
    DrawPicture( fChara_x, VIEW_HEIGHT - 320.0f, &g_tChara );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Scroll_2_1" ),
