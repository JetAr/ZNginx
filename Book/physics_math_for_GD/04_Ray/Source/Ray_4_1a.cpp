...
int MoveRay( void )						// 每帧调用
{
    ...
    v2Present.x = 0.0f;
    v2Present.y = VIEW_HEIGHT / 2;
    fAngle = 0.0f;
    // 一つ目のフォワードandサイドベクトル
    v2Forward.x = SEGMENT_LEN * cosf( fAngle );
    v2Forward.y = SEGMENT_LEN * sinf( fAngle );
    v2Side1.x = v2Forward.y;
    v2Side1.y = -v2Forward.x;
    v2Side1.x *= RAY_WIDTH / 2.0f / SEGMENT_LEN;
    v2Side1.y *= RAY_WIDTH / 2.0f / SEGMENT_LEN;
    for ( i = 0; i < DIVIDE_NUM * 4; i += 4 )
    {
        v2Forward.x = SEGMENT_LEN * cosf( fAngle );
        v2Forward.y = SEGMENT_LEN * sinf( fAngle );
        v2Side2.x = v2Forward.y;
        v2Side2.y = -v2Forward.x;
        v2Side2.x *= RAY_WIDTH / 2.0f / SEGMENT_LEN;
        v2Side2.y *= RAY_WIDTH / 2.0f / SEGMENT_LEN;
        ...
        v2Aim.x = CursorPos.x - v2Present.x;
        v2Aim.y = CursorPos.y - v2Present.y;
...
int Cleanup( void )
{
    ...
    SAFE_RELEASE( g_tPlayer.pSRViewTexture );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Ray_4_1a" ),
