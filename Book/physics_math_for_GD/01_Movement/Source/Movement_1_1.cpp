...
ID3D11Buffer			*g_pD3D11VertexBuffer;	// 顶点缓冲
ID3D11BlendState		*g_pbsAlphaBlend;		// Alpha混合
ID3D11VertexShader		*g_pVertexShader;		// 顶点着色器
ID3D11PixelShader		*g_pPixelShader;		// 像素着色器
ID3D11InputLayout		*g_pInputLayout;		// 着色器输入布局
ID3D11SamplerState		*g_pSamplerState;		// 采样器状态
...
HRESULT Render( void )
{
    ...
    DrawPicture( ( float )x, 0.0f, &g_tBall );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Movement_1_1" ),
