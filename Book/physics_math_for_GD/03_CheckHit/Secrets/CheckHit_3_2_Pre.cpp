...
F_RECT_CIRCLE		rcRectCircleA;		// 矩形＋円Ａ
...
{
    // 線分A
    rcRectCircleA.x  = 60.0f;
    rcRectCircleA.y  = 170.0f;
    rcRectCircleA.vx = 220.0f;
    rcRectCircleA.vy = -110.0f;
    rcRectCircleA.r = 50.0f;
    // 線分B
    rcRectCircleB.x  = 350.0f;
    rcRectCircleB.y  = 160.0f;
    rcRectCircleB.vx = 160.0f;
    rcRectCircleB.vy = 100.0f;
//	rcRectCircleB.vx = 100.0f;  rcRectCircleB.vy = -50.0f;		// 線分AとBが平行な場合をチェック
...
int CheckHit( F_RECT_CIRCLE *prcRectCircle1, F_RECT_CIRCLE *prcRectCircle2 )		// 当たりチェック
{
    int				nResult = false;
    static int		nCount = 0;
...
int DrawRectCircleWithColor( float x1, float y1, float vx, float vy, float r, TEX_PICTURE *pTexPic, int nColor )
{
    ...
    fSide_x = vy / fLength * r;
    fSide_y = -vx / fLength * r;
    // 矩形頂点セット
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "CheckHit_3_2(Secret:線分と線分、起点ソース)" ),
