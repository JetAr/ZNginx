//------------------------------------------------------------
// CheckHit_4_2_Pre.cpp
// 円と扇形(中心空き)の当たり判定、の起点ソース
//
//------------------------------------------------------------
...
int InitShapes( void )						// 最初に１回だけ呼ばれる
{
    crCircleA.x  = 100.0f;
    crCircleA.y = 100.0f;
    crCircleA.r = 60.0f;
    ...
    faFanB.r1 = 100.0f;
    faFanB.r2 = 200.0f;
    faFanB.vx1 = faFanB.r1 * cosf( faFanB.fAngle1 );
    faFanB.vy1 = faFanB.r1 * sinf( faFanB.fAngle1 );
    faFanB.vx2 = faFanB.r1 * cosf( faFanB.fAngle2 );
    faFanB.vy2 = faFanB.r1 * sinf( faFanB.fAngle2 );
...
// 扇形との当たり判定
int IsHitToFan2( F_CIRCLE *pcrCircle, F_FAN *pfaFan )
{
    int				nResult = false;
    static int		nCount = 0;
...
// 絵の扇形描画(色付き、中心空き)
int DrawFan2WithColor( float x, float y, float r1, float r2,
                       float fStartfAngle, float fEndAngle, TEX_PICTURE *pTexPic, int nColor )
{
    int				i;
    int				nDivide_num;
    float			fAngle1, fAngle2;
    ...
    nDivide_num = ( int )( ( fEndAngle - fStartfAngle ) * r2 / 5.0f ) + 1;
    ...
    {
        // 頂点セット
        g_cvVertices[g_nVertexNum + 0].v4Pos   = XMFLOAT4( x + r2 * cosf( fAngle1 ), y + r2 * sinf( fAngle1 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 0].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
        g_cvVertices[g_nVertexNum + 0].v2UV    = XMFLOAT2( 0.0f, 0.0f );//0.5f + 0.5f * cosf( fAngle1 ), 0.5f + 0.5f * sinf( fAngle1 ) );
        g_cvVertices[g_nVertexNum + 1].v4Pos   = XMFLOAT4( x + r1 * cosf( fAngle1 ), y + r1 * sinf( fAngle1 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 1].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
        g_cvVertices[g_nVertexNum + 1].v2UV    = XMFLOAT2( 0.0f, 0.0f );
        g_cvVertices[g_nVertexNum + 2].v4Pos   = XMFLOAT4( x + r1 * cosf( fAngle2 ), y + r1 * sinf( fAngle2 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 2].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
        g_cvVertices[g_nVertexNum + 2].v2UV    = XMFLOAT2( 0.0f, 0.0f );
        g_nVertexNum += 3;
        g_cvVertices[g_nVertexNum + 0].v4Pos   = XMFLOAT4( x + r2 * cosf( fAngle2 ), y + r2 * sinf( fAngle2 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 0].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
        g_cvVertices[g_nVertexNum + 0].v2UV    = XMFLOAT2( 0.0f, 0.0f );//0.5f + 0.5f * cosf( fAngle1 ), 0.5f + 0.5f * sinf( fAngle1 ) );
        g_cvVertices[g_nVertexNum + 1].v4Pos   = XMFLOAT4( x + r2 * cosf( fAngle1 ), y + r2 * sinf( fAngle1 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 1].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
        g_cvVertices[g_nVertexNum + 1].v2UV    = XMFLOAT2( 0.0f, 0.0f );
        g_cvVertices[g_nVertexNum + 2].v4Pos   = XMFLOAT4( x + r1 * cosf( fAngle2 ), y + r1 * sinf( fAngle2 ), 0.0f, 1.0f );
        g_cvVertices[g_nVertexNum + 2].v4Color = XMFLOAT4( fRed, fGreen, fBlue, fAlpha );
...
HRESULT Render( void )
{
    ...
    DrawFan2WithColor( faFanB.x, faFanB.y, faFanB.r1, faFanB.r2, faFanB.fAngle1, faFanB.fAngle2, &g_tRect_A, 0xc0ffffff );
    if ( IsHitToFan2( &crCircleA, &faFanB ) )
    {
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "CheckHit_4_2(Secret:円と中心空き扇形、起点ソース)" ),
