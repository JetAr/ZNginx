...
void KMyCanvas::OnDraw(HDC hDC, const RECT * rcPaint)
{
    ...
    {
    case 0:
        m_Red  = (m_Red   + 0x31) & 0xFF;
        break;
    case 1:
        m_Green= (m_Green + 0x31) & 0xFF;
        break;
    case 2:
        m_Blue = (m_Blue  + 0x31) & 0xFF;
        break;
    }
...
void KMyCanvas::TestClipMeta(HDC hDC, const RECT & rect)
{
    ...
    strcat(mess, "Clip: ");
    if ( m_bValid[1] ) strcat(mess, "Y");
    else strcat(mess, "N");

    strcat(mess, ", Meta: ");
    if ( m_bValid[2] ) strcat(mess, "Y");
    else strcat(mess, "N");

    strcat(mess, ", API: ");
    if ( m_bValid[3] ) strcat(mess, "Y");
    else strcat(mess, "N");
