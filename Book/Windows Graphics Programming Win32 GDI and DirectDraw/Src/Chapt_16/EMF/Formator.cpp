...
void KWinPipe::AddColor(COLORREF color)
{
    ...
    {
        Write("RGB(");
        WriteHex(GetRValue(color));
        Put(',');
        WriteHex(GetGValue(color));
        Put(',');
        WriteHex(GetBValue(color));
        Put(')');
    }
}
...
const long * KWinPipe::Pound(char tag, const long * data, const DicItem * dic)
{
    ...
    {
    case 'B':
        Write("(const BITMAPINFO *) & BMI_");
        WriteDec(m_curBMI); // BITMAPINFO reference
        break;

    case 'b':
        Write("Bits_");
        WriteDec(m_curDIB);  // DIB bits reference
        break;

    case 'C':
        AddBrush((const LOGBRUSH *) data, true);
        len=sizeof(LOGBRUSH);
        break;
    case 'R':
        AddBrush((const LOGBRUSH *) data, false);
        len=sizeof(LOGBRUSH);
        break;
    ...
    case 'o':
        if ( * data > 0 )			  // decimal object index or stock object, move
        {
            Write("hObj[");
            WriteDec(data[0]);
            Put(']');
    ...
    case 'p':
        WriteArray(& data[5], data[4]*2, sizeof(short));
        break;
    case 'P':
        WriteArray(& data[5], data[4]*2, sizeof(long));
        break;
