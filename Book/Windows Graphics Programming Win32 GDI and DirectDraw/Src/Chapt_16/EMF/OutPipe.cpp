...
void KTextPipe::WriteArray(const void * Array, long count, long elmsize, bool decimal)
{
    ...
    for (long i=0; i<count; i++)
    {
        ...
        case sizeof(RECT):
            Write("{ ");
            WriteDec(rArray[i].left);
            Put(',');
            WriteDec(rArray[i].top);
            Put(',');
            WriteDec(rArray[i].right);
            Put(',');
            WriteDec(rArray[i].bottom);
            Write(" }");
            break;
...
const long * KTextPipe::Percent(TCHAR tag, const long * data, const DicItem * dic)
{
    ...
    case 'b':
        WriteDec((long)(* ((unsigned char *) data)));	// signed char
        len = 1;
        break;
    ...
    case 'd':
        ...
        break;  // decimal   signed long
    case 'u':
        WriteDec((unsigned long) data[0]);
        len=4;
        break;  // decimal unsigned long
    case 'x':
        WriteHex(                data[0]);
        len=4;
        break;  // hex     unsigned long

    case 'D':
        WriteDec((long)         (* ((short *)          data)));
        len=2;
        break; // signed short
    case 'U':
        WriteDec((unsigned long)(* ((unsigned short *) data)));
        len=2;
        break; // signed short
    case 'X':
        WriteHex(                * ((unsigned short *) data));
        len=2;
        break; // unsigned short

    case 'S':
        WriteString(data, -1, true);
        break; // wide string
    case 's':
        WriteString(data, -1, false);
        break; // ascii string
    ...
    case 'n':
        m_nCount++;					// sequence count: 1 1 2 2 ...
        WriteDec((long)(m_nCount/2));
        break;

    case 'm':
        m_mCount++;					// sequence count: 1 1 2 2 ...
