...
HTREEITEM AddCharWidth(KTreeView & Tree, HDC hDC, const TCHAR * name, HTREEITEM hLast, int typ)
{
    ...
    {
    case 0:
        GetCharWidth32(hDC, uFirst, uLast, intWidth);
        break;
    case 1:
        GetCharWidthFloat(hDC, uFirst, uLast, fltWidth);
        break;
    case 2:
        GetCharABCWidths(hDC, uFirst, uLast, abcWidth);
        break;
    case 3:
        GetCharABCWidthsFloat(hDC, uFirst, uLast, abcfWidth);
        break;
    }
    ...
    for (UINT ch=uFirst; ch<=uLast; ch++)
    {
        ...
        {
        case 0:
            sprintf(num, "%d",    intWidth[ch-uFirst]);
            break;
        case 1:
            sprintf(num, "%7.3f", fltWidth[ch-uFirst]);
            break;
        case 2:
            sprintf(num, "%d %d %d", abcWidth[ch-uFirst].abcA, abcWidth[ch-uFirst].abcB, abcWidth[ch-uFirst].abcC);
            break;
        case 3:
            sprintf(num, "%7.3f %7.3f %7.3f", abcfWidth[ch-uFirst].abcfA, abcfWidth[ch-uFirst].abcfB, abcfWidth[ch-uFirst].abcfC);
            break;
        }
