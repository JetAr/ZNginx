...
void DecodeDC(HDC hDC, unsigned * addr)
{
    ...
    SetBrushOrgEx(hDC, 1, 2, NULL);
    dc.Save();
    SetBrushOrgEx(hDC, 3, 4, NULL);
    dc.Comp("SetBrushOrgEx");

    SetGraphicsMode(hDC, GM_COMPATIBLE);
    dc.Save();
    SetGraphicsMode(hDC, GM_ADVANCED);
    dc.Comp("SetGraphicsMode");
...
//	FLOATOBJ2Double(p->XFORM_eDx);
    ...
    SetWorldTransform(hDC, & xform2);
    dc.Comp("SetWorldTransform 1");

//	FLOATOBJ2Double(p->XFORM_eM11);

    dc.Save();
    XFORM xform3= { 1.1111f, 2.2222f, 0xFF03, 0xFF04, 0xFF05, 0xFF06 };
    SetWorldTransform(hDC, & xform3);
    dc.Comp("SetWorldTransform 2");

    dc.Save();
    XFORM xform4= { 1.1111f, 2.2222f, 3.333f, 0xFF04, 0xFF05, 0xFF06 };
    SetWorldTransform(hDC, & xform4);
    dc.Comp("SetWorldTransform 3");

    dc.Save();
    XFORM xform5= { 1.1111f, -2.2222f, 3.333f, 4.4444f, 0xFF05, 0xFF06 };
    SetWorldTransform(hDC, & xform5);
    dc.Comp("SetWorldTransform 4");

    dc.Save();
    XFORM xform6= { 1.1111f, 2.2222f, 3.333f, 4.4444f, 5.5555f, 0xFF06 };
    SetWorldTransform(hDC, & xform6);
    dc.Comp("SetWorldTransform 5");

    dc.Save();
    XFORM xform7= { 1.1111f, 2.2222f, 3.333f, 4.4444f, 5.5555f, 6.6666f };
    SetWorldTransform(hDC, & xform7);
    dc.Comp("SetWorldTransform 6");


    SetMiterLimit(hDC, 0.5, NULL);
    dc.Save();
    SetMiterLimit(hDC, 1.1f, NULL);
    dc.Comp("SetMiterLimit");

    SetArcDirection(hDC, AD_COUNTERCLOCKWISE );
    dc.Save();
    SetArcDirection(hDC, AD_CLOCKWISE);
    dc.Comp("SetArcDirection");

    RECT rect = { 1961, 1962, 1963, 1964 };
    dc.Save();
    SetBoundsRect(hDC, & rect, DCB_ENABLE);
    dc.Comp("SetBoundsRect");
    ...
    dc.Save();
    SelectObject(hDC, CreateHalftonePalette(hDC));
    dc.Comp("SelectPalette");
    ...
    dc.Comp("SetStretchBltMode");

    SetTextColor(hDC, RGB(3, 0, 0));
    dc.Save();
    SetTextColor(hDC, RGB(4, 0, 0));
    dc.Comp("SetTextColor");
    ...
    SetViewportOrgEx(hDC, 1, 2, NULL);
    dc.Save();
    SetViewportOrgEx(hDC, 3, 4, NULL);
    dc.Comp("SetViewportOrgEx");

    SetViewportExtEx(hDC, 100, 200, NULL);
    dc.Save();
    SetViewportExtEx(hDC, 300, 400, NULL);
    dc.Comp("SetViewportExtEx");

    SetWindowOrgEx(hDC, 1, 3, NULL);
    dc.Save();
    SetWindowOrgEx(hDC, 2, 4, NULL);
    dc.Comp("SetWindowOrgEx");

    SetWindowExtEx(hDC, 100, 50, NULL);
    dc.Save();
    SetWindowExtEx(hDC, 50, 100, NULL);
    dc.Comp("SetWindowExtEx");

    SetTextAlign(hDC, TA_BASELINE);
    dc.Save();
    SetTextAlign(hDC, TA_BOTTOM);
    dc.Comp("SetTextAlign");

    SetTextJustification(hDC, 1, 2);
    dc.Save();
    SetTextJustification(hDC, 3, 4);
    dc.Comp("SetTextJustification");

    SetTextCharacterExtra(hDC, 5);
    dc.Save();
    SetTextCharacterExtra(hDC, 0);
    dc.Comp("SetTextCharacterExtra");

    SetMapperFlags(hDC, 1);
    dc.Save();
    SetMapperFlags(hDC, 0);
    dc.Comp("SetMapperFlags");
...
void KDecodeTablePage::ShowDetails(unsigned nIndex)
{
    ...
    message.Append("Type: ");
    message.Append(GetObjectTypeName(hObj));
    message.Append("\r\n");
