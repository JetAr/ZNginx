#ifdef ____SYNCHRONIZED_H_CYCLE__
#error Cyclic dependency discovered: _synchronized.h
#endif

#ifndef ___SYNCHRONIZED_H__


#ifndef DOXYGEN

#define ___SYNCHRONIZED_H__
#define ___SYNCHRONIZED_H_CYCLE__
...
/** \file
    Defines a syntax for a SYNCHRONIZED statement blocks.
...
/// <summary>
/// Synchronized blocks guaranty that a code inside a block is executed synchronously.
/// </summary>
/// <remarks>
...
/// </remarks>
#define SYNCHRONIZED                                                \
    static long _WCP_UNIQUE_LOCAL_SCOPE_NAME(lock, __LINE__) = 0;   \
    _WCP_DECLARE_STATEMENT_BLOCK(WCP_NS::internals::                \
        synchronized_statement_block_expression(                    \
        _WCP_UNIQUE_LOCAL_SCOPE_NAME(lock, __LINE__)))
...
namespace internals
{
class synchronized_statement_block_expression
...
public:
    ...
    synchronized_statement_block_expression(long& counter)
        : counter(counter)
    {
        if(InterlockedIncrement(&counter) > 1)
        {
            Sleep(WCP_SYNC_DELAY);
...
#undef ___SYNCHRONIZED_H_CYCLE__
#endif /*___SYNCHRONIZED_H__*/
