...
static void Tensurereportfile(void)
{
    ...
    if (!Treportfile) Tfailed("fdopen ADNS_TEST_REPORT_FD");
}
...
static void Tensureinputfile(void)
{
    ...
    if (fdstr)
    {
        ...
        if (!Tinputfile) Tfailed("fdopen ADNS_TEST_IN_FD");
    }
...
static int Pbytes(byte *buf, int maxlen)
{
    ...
    for (;;)
    {
        ...
        if (!pf) Psyntax("invalid first hex digit");
        v= (pf-hexdigits)<<4;
        c= getc(Tinputfile);
        Pcheckinput();
        pf= strchr(hexdigits,c);
        if (!pf) Psyntax("invalid second hex digit");
        v |= (pf-hexdigits);
