...
bool LoadTGA(Texture * texture, const TCHAR * filename)				// Load a TGA file
{
	...
	errno_t err = _tfopen_s(&fTGA, filename, _T("rb"));								// Open file for reading

	if(err != 0 || fTGA == NULL)											// If it didn't open....
	{
		MessageBoxA(NULL, "Could not open texture file", "ERROR", MB_OK);	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		MessageBoxA(NULL, "Could not read file header", "ERROR", MB_OK);		// If it fails, display an error message 
	...
	{																		// an Uncompressed TGA image
		return LoadUncompressedTGA(texture, filename, fTGA);						// If so, jump to Uncompressed TGA loading code
	}
	else if(memcmp(cTGAcompare, &tgaheader, sizeof(tgaheader)) == 0)		// See if header matches the predefined header of
	{																		// an RLE compressed TGA image
		return LoadCompressedTGA(texture, filename, fTGA);							// If so, jump to Compressed TGA loading code
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{
		MessageBoxA(NULL, "Invalid texture information", "ERROR", MB_OK);	// Display Error
	...
	{
		MessageBoxA(NULL, "Could not allocate memory for image", "ERROR", MB_OK);	// Display Error
...
{ 
	...
	{
		MessageBoxA(NULL, "Invalid texture information", "ERROR", MB_OK);	// If it isnt...Display error
	...
	do
	{
		...
		{
			MessageBoxA(NULL, "Could not read RLE header", "ERROR", MB_OK);	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					MessageBoxA(NULL, "Could not read image data", "ERROR", MB_OK);		// IF we cant, display an error
				...
				{
					MessageBoxA(NULL, "Too many pixels read", "ERROR", NULL);			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{	
				MessageBoxA(NULL, "Could not read from file", "ERROR", MB_OK);			// If attempt fails.. Display error (again)
