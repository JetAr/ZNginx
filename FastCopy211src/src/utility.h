...
protected:
    struct PathObj : THashObj
    {
        void	*path;
        int		len;
        PathObj(const void *_path, int len=-1)
        {
            Set(_path, len);
        }
        ~PathObj()
...
public:
    ...
    void	*Path(int idx) const
    {
        return idx < num ? pathArray[idx]->path : NULL;
    }
    int		Num(void) const
    {
        return	num;
    ...
    u_int	MakeHashId(const PathObj *obj)
    {
        return MakeHash(obj->path, obj->len * CHAR_LEN_V);
    }
};
...
protected:
    ...
    int		LetterToIndex(int drvLetter)
    {
        return toupper(drvLetter) - 'A';
    }
...
public:
    ...
    BOOL IsWait()
    {
        return cv.WaitThreads() ? TRUE : FALSE;
    }
    void Notify()
    {
        cv.Notify();
    ...
    int Grow(int grow_size)
    {
        return buf.Grow(grow_size);
    }
    int MinMargin()
    {
        return min_margin;
