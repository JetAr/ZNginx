...
static int icmp_unreach_rtt(void *quoted_ip, int size, int *seqp,
                            float *ms_delay);
...
#else
void log_traceroute(void *packet2, int size, int icmp_code)
#endif
{
...
#else
int recv_icmp(void *packet2, size_t size)
#endif
{
    struct myicmphdr icmp;
