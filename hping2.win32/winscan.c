...
extern struct timeb msec_delay;
...
/* -------------------------------- output ---------------------------------- */
DWORD WINAPI sender(LPVOID pi2)
{
    int i, retry = 0;
    time_t start_time;
    struct portinfo *pi = (struct portinfo *)pi2;
    ...
    while(1)
    {
        ...
        for (i = 0; i < MAXPORT; i++)
        {
            if (pi[i].active && pi[i].retry)
            {
                ...
                if (opt_waitinusec)
                {
                    if (msec_delay.millitm)
                        sleep(msec_delay.millitm);
                }
                else
                {
                    sleep(sending_wait);
...
/* -------------------------------- input  ---------------------------------- */
static void receiver(struct portinfo *pi, HANDLE child)
{
    struct myiphdr ip;
