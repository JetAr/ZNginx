...
static struct ago_optlist hping_optlist[] =
{
    ...
    { '\0', "route-help",   OPT_ROUTE_HELP, AGO_NOARG }, AGO_LIST_TERM,
...
#else
            if ((ip = inet_addr(str + i)) != INADDR_NONE)
            {
                memcpy(route + 4 * n, &ip, 4);
#endif
                n++;
                if (c == '/')
                    str[j++] = '/';
...
int parse_options(int argc, char **argv)
{
    ...
    while ((o = antigetopt(argc, argv, hping_optlist)) != AGO_EOF)
    {
        ...
        case OPT_INTERVAL:
            ...
            {
                opt_waitinusec = TRUE;
                msec_delay.millitm = atol(ago_optarg + 1);
        ...
        case OPT_LSRR:
            ...
            if (lsr[0])
                printf("Warning: erasing previously given loose source route");
            lsr[0] = 131;
            break;
        case OPT_SSRR:
            opt_ssrr = TRUE;
            ...
            if (ssr[0])
                printf("Warning: erasing previously given strong source route");
            ssr[0] = 137;
            break;
        case OPT_ROUTE_HELP:
            route_help();
    ...
    {
#else
    else if (opt_waitinusec == TRUE && msec_delay.millitm < 0)
    {
#endif
        printf("Option error: bad timing interval\n");
        exit(1);
