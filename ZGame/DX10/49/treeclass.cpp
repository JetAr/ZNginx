...
bool TreeClass::Initialize(ID3D10Device* device, char* trunkModelFilename, WCHAR* trunkTextureFilename, char* leafModelFilename, WCHAR* leafTextureFilename, float scale)
{
    bool result;
...
void TreeClass::RenderTrunk(ID3D10Device* device)
{
    // Put the vertex and index buffers on the graphics pipeline to prepare them for drawing.
    RenderTrunkBuffers(device);

    return;
}


void TreeClass::RenderLeaves(ID3D10Device* device)
{
    // Put the vertex and index buffers on the graphics pipeline to prepare them for drawing.
    RenderLeafBuffers(device);
...
ID3D10ShaderResourceView* TreeClass::GetTrunkTexture()
{
    return m_TrunkTexture->GetTexture();
}


ID3D10ShaderResourceView* TreeClass::GetLeafTexture()
{
    return m_LeafTexture->GetTexture();
}


bool TreeClass::InitializeTrunkBuffers(ID3D10Device* device, float scale)
{
    VertexType* vertices;
...
bool TreeClass::InitializeLeafBuffers(ID3D10Device* device, float scale)
{
    VertexType* vertices;
    unsigned long* indices;
    D3D10_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
...
void TreeClass::RenderTrunkBuffers(ID3D10Device* device)
{
    unsigned int stride;
    unsigned int offset;
    ...
    device->IASetVertexBuffers(0, 1, &m_trunkVertexBuffer, &stride, &offset);

    // Set the index buffer to active in the input assembler so it can be rendered.
    device->IASetIndexBuffer(m_trunkIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
...
void TreeClass::RenderLeafBuffers(ID3D10Device* device)
{
    unsigned int stride;
    unsigned int offset;
    ...
    // Set the vertex buffer to active in the input assembler so it can be rendered.
    device->IASetVertexBuffers(0, 1, &m_leafVertexBuffer, &stride, &offset);

    // Set the index buffer to active in the input assembler so it can be rendered.
    device->IASetIndexBuffer(m_leafIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
...
bool TreeClass::LoadTextures(ID3D10Device* device, WCHAR* trunkFilename, WCHAR* leafFilename)
{
    bool result;
