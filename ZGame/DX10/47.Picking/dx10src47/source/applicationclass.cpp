...
bool ApplicationClass::Render()
{
    ...
    m_LightShader->Render(m_D3D->GetDevice(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Model->GetTexture(), m_Light->GetDirection());
    ...
    result = m_Bitmap->Render(m_D3D->GetDevice(), mouseX, mouseY);
    if(!result)
    {
        return false;
    }
...
void ApplicationClass::TestIntersection(int mouseX, int mouseY)
{
    ...
    if(intersect == true)
    {
        ...
        result = m_Text->SetIntersection(true);
    }
    else
    {
        // If not then set the intersection to "No".
        result = m_Text->SetIntersection(false);
    }
