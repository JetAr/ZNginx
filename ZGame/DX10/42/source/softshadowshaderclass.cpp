...
bool SoftShadowShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
    bool result;


    // Initialize the shader that will be used to draw the triangles.
    result = InitializeShader(device, hwnd, L"../Engine/softshadow.fx");
    if(!result)
    {
        return false;
    }
...
void SoftShadowShaderClass::Render(ID3D10Device* device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
                                   ID3D10ShaderResourceView* texture, ID3D10ShaderResourceView* shadowTexture, D3DXVECTOR3 lightPosition,
                                   D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor)
{
    // Set the shader parameters that it will use for rendering.
    SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, shadowTexture, lightPosition, ambientColor, diffuseColor);
...
bool SoftShadowShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
    HRESULT result;
    ...
    m_technique = m_effect->GetTechniqueByName("SoftShadowTechnique");
    if(!m_technique)
    {
        return false;
    }
    ...
    m_shadowTexturePtr = m_effect->GetVariableByName("shadowTexture")->AsShaderResource();
...
void SoftShadowShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
        ID3D10ShaderResourceView* texture, ID3D10ShaderResourceView* shadowTexture, D3DXVECTOR3 lightPosition,
        D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor)
{
    ...
    m_shadowTexturePtr->SetResource(shadowTexture);
...
void SoftShadowShaderClass::RenderShader(ID3D10Device* device, int indexCount)
{
