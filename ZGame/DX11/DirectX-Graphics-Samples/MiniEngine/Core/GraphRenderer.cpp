...
public:
    ...
    Color GetColor(GraphHandle GraphID)
    {
        return m_Graphs[GraphID]->m_Color;
    }
    uint32_t Size()
    {
        return (uint32_t)m_Graphs.size();
    }
    uint32_t GetActiveGraphCount()
    {
        return m_ActiveGraphs;
    }

    float* GetPresetMax()
    {
        return m_PresetMax.get();
    }
