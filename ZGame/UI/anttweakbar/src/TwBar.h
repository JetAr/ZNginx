...
struct CTwVarAtom : CTwVar
{
    ...
    virtual void            SetReadOnly(bool _ReadOnly)
    {
        ...
        if( m_Type!=TW_TYPE_BUTTON && m_SetCallback==NULL && m_Ptr==NULL ) m_ReadOnly=true;
    }
    virtual bool            IsReadOnly() const
    {
        if( m_Type!=TW_TYPE_BUTTON && m_SetCallback==NULL && m_Ptr==NULL ) return true;
        else return m_ReadOnly;
    }
...
struct CTwVarGroup : CTwVar
{
    ...
    virtual void            SetReadOnly(bool _ReadOnly)
    {
        for(size_t i=0; i<m_Vars.size(); ++i) if(m_Vars[i]) m_Vars[i]->SetReadOnly(_ReadOnly);
    }
    virtual bool            IsReadOnly() const
    {
        for(size_t i=0; i<m_Vars.size(); ++i) if(m_Vars[i] && !m_Vars[i]->IsReadOnly()) return false;
        return true;
    }
    CTwVarGroup()
    {
...
struct CTwBar
{
    ...
    {
        m_HighlightedLine = -1;    // used by PopupCallback
        NotUpToDate();
    }
    void                    SetFocus(bool _Focus)
    {
        m_DrawHandles = _Focus;    // used by PopupCallback
    }
    bool                    GetFocus() const
    {
        return m_DrawHandles || m_EditInPlace.m_Active;
    }
    void                    StopEditInPlace()
    {
        if( m_EditInPlace.m_Active ) EditInPlaceEnd(false);
    }
