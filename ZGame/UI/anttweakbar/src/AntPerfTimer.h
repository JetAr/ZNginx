...
struct PerfTimer
{
    inline        PerfTimer()
    {
        if( !QueryPerformanceFrequency(&Freq) ) MessageBox(NULL, _T("Precision timer not supported"), _T("Problem"), MB_ICONEXCLAMATION);
        Reset();
    }
    inline void   Reset()
    {
    ...
    {
        if( QueryPerformanceCounter(&End) ) return ((double)End.QuadPart - (double)Start.QuadPart)/((double)Freq.QuadPart);
        else return 0;
    }
protected:
