...
static GLuint CompileShader(GLuint shader)
{
    ...
    if (status == GL_FALSE)
    {
        ...
        _glGetShaderInfoLog(shader, sizeof(strInfoLog), NULL, strInfoLog);
        CHECK_GL_ERROR;
#ifdef ANT_WINDOWS
...
static GLuint LinkProgram(GLuint program)
{
    ...
    if (status == GL_FALSE)
    {
        ...
        GLchar strInfoLog[256];
        _glGetProgramInfoLog(program, sizeof(strInfoLog), NULL, strInfoLog);
        CHECK_GL_ERROR;
#ifdef ANT_WINDOWS
...
int CTwGraphOpenGLCore::Shut()
{
    ...
    _glDeleteProgram(m_LineRectProgram);
    m_LineRectProgram = 0;
    _glDeleteShader(m_LineRectVS);
    m_LineRectVS = 0;
    _glDeleteShader(m_LineRectFS);
    m_LineRectFS = 0;

    _glDeleteProgram(m_TriProgram);
    m_TriProgram = 0;
    _glDeleteShader(m_TriVS);
    m_TriVS = 0;

    _glDeleteProgram(m_TriUniProgram);
    m_TriUniProgram = 0;
    _glDeleteShader(m_TriUniVS);
    m_TriUniVS = 0;

    _glDeleteProgram(m_TriTexProgram);
    m_TriTexProgram = 0;
    _glDeleteShader(m_TriTexVS);
    m_TriTexVS = 0;
    _glDeleteShader(m_TriTexFS);
    m_TriTexFS = 0;

    _glDeleteProgram(m_TriTexUniProgram);
    m_TriTexUniProgram = 0;
    _glDeleteShader(m_TriTexUniVS);
    m_TriTexUniVS = 0;

    _glDeleteBuffers(1, &m_LineRectVertices);
    m_LineRectVertices = 0;
    _glDeleteBuffers(1, &m_LineRectColors);
    m_LineRectColors = 0;
    _glDeleteVertexArrays(1, &m_LineRectVArray);
    m_LineRectVArray = 0;

    _glDeleteBuffers(1, &m_TriVertices);
    m_TriVertices = 0;
    _glDeleteBuffers(1, &m_TriColors);
    m_TriColors = 0;
    _glDeleteBuffers(1, &m_TriUVs);
    m_TriUVs = 0;
    _glDeleteVertexArrays(1, &m_TriVArray);
    m_TriVArray = 0;
...
void CTwGraphOpenGLCore::BeginDraw(int _WndWidth, int _WndHeight)
{
    ...
    _glGetIntegerv(GL_VIEWPORT, m_PrevViewport);
    CHECK_GL_ERROR;
    ...
    m_PrevVArray = 0;
    _glGetIntegerv(GL_VERTEX_ARRAY_BINDING, (GLint*)&m_PrevVArray);
    CHECK_GL_ERROR;
    ...
    _glGetIntegerv(GL_SCISSOR_BOX, m_PrevScissorBox);
    CHECK_GL_ERROR;

    _glGetIntegerv(GL_BLEND_SRC, &m_PrevSrcBlend);
    CHECK_GL_ERROR;
    _glGetIntegerv(GL_BLEND_DST, &m_PrevDstBlend);
    CHECK_GL_ERROR;
    ...
    _glGetIntegerv(GL_CURRENT_PROGRAM, (GLint*)&m_PrevProgramObject);
    CHECK_GL_ERROR;
    _glBindVertexArray(0);
    CHECK_GL_ERROR;
    _glUseProgram(0);
    ...
    _glGetIntegerv(GL_ACTIVE_TEXTURE, (GLint*)&m_PrevActiveTexture);
    CHECK_GL_ERROR;
...
void CTwGraphOpenGLCore::EndDraw()
{
    ...
    _glScissor(m_PrevScissorBox[0], m_PrevScissorBox[1], m_PrevScissorBox[2], m_PrevScissorBox[3]);
    CHECK_GL_ERROR;

    _glBlendFunc(m_PrevSrcBlend, m_PrevDstBlend);
    CHECK_GL_ERROR;

    _glBindTexture(GL_TEXTURE_2D, m_PrevTexture);
    CHECK_GL_ERROR;

    _glUseProgram(m_PrevProgramObject);
    CHECK_GL_ERROR;

    _glBindVertexArray(m_PrevVArray);
    CHECK_GL_ERROR;

    _glViewport(m_PrevViewport[0], m_PrevViewport[1], m_PrevViewport[2], m_PrevViewport[3]);
    CHECK_GL_ERROR;
