...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pSceneLayout ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( quadlayout, 2, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pQuadLayout ) );
...
void LoadDepthStencilStates(ID3D10Device* pd3dDevice)
{
    ...
    for( UINT i=0; i<MAX_DEPTH_STENCIL_MODES; i++ )
    {
        ...
        g_SampleUI.GetComboBox(IDC_SCENEDEPTHSTENCIL_MODE)->AddItem( g_szDepthStencilModes[i], (void*)(UINT64)i );
    }
}
