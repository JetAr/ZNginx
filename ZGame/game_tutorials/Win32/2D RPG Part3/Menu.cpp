...
void DrawInventoryScreen()
{
    ...
    for(int i = 0; i < g_Player.GetInventorySize(); i++)
    {
        ...
        if(i >= kMaxItemHeight)
        {
            ...
            // then times that by the max item width to move over to a new column.  That
            // result is then multiplied by the width of a tile.  To get the y, get the
            // same info with the index divided by the max height, then times that by the
            // max height.  We then subtract that value from the current index (i), then
            // multiply that by the height of the font, with an extra padding of kTileHeight.
            // If it's hard at first to wrap your head around math like this, it's okay.
            // Even before programming things like this we need to sit and think about it
            // for a bit to see what we need as well :)  Just throw some test values in
            // their and you will understand why it formats the way we want it to.
