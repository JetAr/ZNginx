...
///////////////////////////////////////////////////////////////////////////////////
//
// In this tutorial version added 4 new functions to allow us to Load and Save
// our maps: FindHBitmap() - Load() - Save() - SetTileInfo().
//
//
...
HBITMAP CMap::FindHBitmap(char *szFile, int type)
{
    ...
    // memory, we use this function to search the global tile lists and
    // gives us back an HBITMAP with the same file name.  We then share that
    // between the map tile lists and the tool bar tile lists.
...
void CMap::Load(char *szFile)
{
    ...
    // In this function we want to load all of the data from a specified map.
    // We do this by first opening the file, then clearing our map tile lists.
    // We read the tile size for each list so we know how many tiles of that
    // type to read in.  Using our vector lists it's easy to allocate the
    // memory we need for each list.  Lastly, we go through the global lists
    // and get a bitmap handle to each tile that we load.  This way we save
    // some memory and don’t need to worry about freeing more memory.
    // (* IMPORTANT *) It's probably common sense, but you have to make sure
    // you load the file the SAME exact order it was saved.  For instance, if
    // you load the map tiles first, you can save the items first.  "Capish"?
...
void CMap::Save(char *szFile)
{
    ...
    // Just like the Load() function, to save the map we will be doing pretty
    // much the same things, except instead of fread() we will use fwrite() to
    // write the information to disk for our .map file.
...
void CMap::SetTileInfo(CTile *pTile, char *szFile)
{
    ...
    // You will notice that there is text files in the directories of our tiles.
    // These info files store the stats for items, monsters, etc...  This function
    // loads all that information.  If you don't want to store the information this
    // way you can make a database, or just store all that information in code and
    // look it up in code.  It depends on how well you want to allow the end users
    // to mess or change your data.  This way people can add their own items and
    // monsters without touching any code.
    ...
    while(!feof(fp))
    {
        if(pTile->GetType() == ITEM)				// If the tile type is an item
        {
            ...
            // Notice that we use a _strlwr() function to make both checks not case sensitive.
...
// *Quick Notes*
//
// In this file we added 4 new functions to load and save files.
//
// The next tutorial we will add exit setting functionality.
//
//
