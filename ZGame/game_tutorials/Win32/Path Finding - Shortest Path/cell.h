...
// Here's the defines for the different values a CELL can have
...
// It's totally valid to put all the implementation in the header file.  Because this class
// only contains "get" and "set" methods (besides the default constructor) which are no
// more than a line of code, it's more tidy and convenient to put the implementation in the
// header file. Thus, that's what we're doing %)
class CCell
{
public:
