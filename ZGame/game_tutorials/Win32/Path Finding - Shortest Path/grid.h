...
#define ROW_MAX 10 // Number of rows in our GRID
#define COL_MAX 15 // Number of columns in our GRID

// This is our GRID class -- It handles all the drawing of the grid AND the actual path
// finding -- You'll notice we use a double array of CCell's to make the GRID.  Whenever you
// deal with double arrays, it's important to keep how you index in to it straight.  At the
// bottom of this file you'll see the "double array layout" and the "Win32 coordinate layout"
// of a 3x3 GRID -- This should help clear up in your mind the row/column flip-flopping that
// goes on when dealing with a 2D array.
//
// It is important to note that our GRID is based on the Win32 coordinate system.  So
// when we pass in an (x,y) it should be passed in using the Win32 coordinate system -- We'll do
// the conversion to 2D array coordinates (ie flip-flopping them) internally
//
class CGrid
{
public:
    ...
    // Return width or height in pixels of GRID
    int getWidth()
    {
        return CCell::width * COL_MAX;
    }
    int getHeight()
    {
        return CCell::height * ROW_MAX;
    }
    ...
    bool pathFind(); // Finds the shortest path -- Returns true
    // on success (ie a path existed), false otherwise
    ...
    bool doneDrawing()
    {
        return path_nodes.empty() && dest_found;    // Slightly tricky
    }
    // syntax here but
    // it's a good mental
    // exercise to figure
...
private:
    ...
    CCell grid_array[ROW_MAX][COL_MAX]; // The array that makes up the grid

    bool dest_found; // This gets set to true when we path find to our
    // destination

    // We will use this to store the (x,y) indexes (2D array index)
    // into our GRID that creates the path from the start location to the
    // end location -- When we are done filling "path_nodes" the start
    // location (starting cell) will be the at the TOP of the stack and the
    // destination location (destination cell) will be at the BOTTOM of the stack
