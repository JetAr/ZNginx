...
void DrawCursorTile()
{
    ...
    //z 获取 cursor 坐标并且将其转换为client coordinates 位置。而非 screen coordinates。
    ...
    // Otherwise draw a transparent bitmap for the rest of the tile tiles (items, monsters, etc...)
        ...
        //z 诸如 item monster 等，都使用透明绘制来处理。
    ...
    if(g_bLButtonDown)
    {
        ...
        // 非普通素材一次按下只能 insert 一次，不能持续 inert。
        if(g_pCurrentMap->GetCurrentType() != TILE_TYPE)
            g_bLButtonDown = 0;
    }
}
...
void HandleScrollBar(WPARAM wParam)
{
    ...
    {
    case SB_LINEUP:
    case SB_PAGEUP:					// If we choose the up button or click above the middle button
        g_scrollBarPosY = g_scrollBarPosY - 1;		// Decrease the scroll bar position
        break;

    case SB_LINEDOWN:
    case SB_PAGEDOWN:				// If we choose the down button or click below the middle button
        g_scrollBarPosY = g_scrollBarPosY + 1;		// Increase the scroll bar position
        break;

    case SB_THUMBPOSITION:
    case SB_THUMBTRACK:		// If we dragged the middle button (thumb)
