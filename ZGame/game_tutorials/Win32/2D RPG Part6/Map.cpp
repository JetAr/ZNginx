...
// Since we are going to be saving the player's inventory, we added 2 save and load
// functions to our CItem class, which saves an item to a file, give the file pointer.
...
void CMap::Draw()
{
    ...
    // Go through all of the party and draw each player
...
void CMap::CheckForNpc(int x, int y)
{
    ...
    for(int i = 0; i < (int)m_vNpcs.size(); i++)
    {
        ...
        if(x == pNpc->GetIndex().x && y == pNpc->GetIndex().y)
        {
            ...
            // Check to see if this NPC has an action key attached to it
...
void CMap::CheckForItem(int x, int y)
{
    ...
    for(int i = 0; i < (int)m_vItems.size(); i++)
    {
        ...
        if(x == pItem->GetIndex().x && y == pItem->GetIndex().y)
        {
            ...
            // Handle any special event with an item's action key
...
void CMonster::Attack(CPlayer *pPlayer)
{
    ...
    // Here we check who is stronger, the player or the monster, then calculate a damage ratio
...
///////////////////////////////// SAVE ITEM \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This saves an item to a file, given the file pointer
/////
///////////////////////////////// SAVE ITEM \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

void CItem::SaveItem(FILE *fp)
{
    fwrite(&m_itemType, sizeof(int), 1, fp);
    fwrite(&m_lifeInc, sizeof(int), 1, fp);
    fwrite(&m_strengthInc, sizeof(int), 1, fp);
    fwrite(&m_protectionInc, sizeof(int), 1, fp);
    fwrite(m_szItem, sizeof(char), kMaxNameLen, fp);
}


///////////////////////////////// LOAD ITEM \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This loads an item from a file, given a file pointer
/////
///////////////////////////////// LOAD ITEM \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

void CItem::LoadItem(FILE *fp)
{
    fread(&m_itemType, sizeof(int), 1, fp);
    fread(&m_lifeInc, sizeof(int), 1, fp);
    fread(&m_strengthInc, sizeof(int), 1, fp);
    fread(&m_protectionInc, sizeof(int), 1, fp);
    fread(m_szItem, sizeof(char), kMaxNameLen, fp);
}
...
// *Quick Notes*
//
// The LoadItem() and SaveItem() were added to this file.  This
// is used when we save or load our game files.  Since we need to
// save and load the player's inventory, these were necessary.
//
// Let's now jump over to the Menu.cpp file to check the small
// changes that were added to be able to have load and save support
// in the main menu.
//
//
