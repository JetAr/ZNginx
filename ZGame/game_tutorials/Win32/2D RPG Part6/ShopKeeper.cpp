...
{
    // Get the total stats for the item (not hp stats though)
...
void ItemPromptSell()
{
    ...
    // Drop the item from the player's inventory, then erase it from the map
...
void DrawBuySellScreen(vector<CItem> *pItemList)
{
    ...
    // Go through the items and draw them in the inventory menu box
    ...
    // Redraw the screen to show the results
    g_Buffer.SwapBackBuffer(FALSE);
}
