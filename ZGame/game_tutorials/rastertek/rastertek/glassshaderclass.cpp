...
{
    bool result;
    result = InitializeShader(device, hwnd, L"../Engine/glassvs.hlsl", L"../Engine/glassps.hlsl");
    if (FAILED(result))
    {
        return false;
    }
...
bool GlassShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    ...
    result = D3DX11CompileFromFile(psFilename, NULL, NULL, "GlassPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS,
