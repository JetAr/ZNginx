...
// a (nx, ny, nz) normal to the vertex and a color
#define SVertexType D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_DIFFUSE

// Our vertex struct
struct SVertex
{
    float x, y, z; // Our position in space (world coordinates)
    float nx, ny, nz; // Our vertex normal
    DWORD color; // Our diffuse color
};

// D3D's FVF Vertex Order
