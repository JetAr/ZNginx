...
// These define the three vectors which make up our world coordinate system
// We will use the default, left-handed, Direct3D coordinate system
const CVector kWorldXAxis(1,0,0);
const CVector kWorldYAxis(0,1,0);
const CVector kWorldZAxis(0,0,1);
...
public:
    ...
    static bool render(const SVertex *vertList, int numVerts);

    // Renders a indexed list of SVertex as triangles
    static bool render(const SVertex *vertList, int numVerts, const WORD *indexList, int numIndices);

    // Renders a list of SVertex as line segments
    static bool renderLine(const SVertex *vertList, int numVerts);
