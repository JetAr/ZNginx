...
void CD3DMesh::render() const
{
    ...
    // Select the texture for the mesh
    mTexture->select();
...
{
    // Safety first!  If the input paramter is NULL or the
    // texture has already been loaded, return false
    if((!fileName) || (mTexture != NULL))
        return false;
...
// Set the vertex data
bool CD3DMesh::setVertexData(const SVertex *verts, int vertCount)
{
    // Safety first!  If the input parameters are bogus or
    // the vertex buffer is already set, return false
    if((!verts) || (vertCount <= 0) || (mVertexBuffer != NULL))
        return false;
...
// Set the index data
bool CD3DMesh::setIndexData(const WORD *indices, int indexCount)
{
    // Safety first!  If the input values are bogus or
    // the index data is already set, return false
    if((!indices) || (indexCount <= 0) || (mIndexBuffer != NULL))
        return false;
