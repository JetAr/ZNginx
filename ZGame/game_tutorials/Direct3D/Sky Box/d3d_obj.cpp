...
void CD3DObj::setDefaultRenderState()
{
    ...
    // **NOTE** We are setting this to D3DTEXF_POINT since the only textured object
    //			we render is a skybox and it needs this type of filtering
    for(unsigned int i = 0; i < caps.MaxTextureBlendStages; ++i)
    {
        mResult = mDevice->SetSamplerState(i, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
        assert(mResult == D3D_OK);

        // Set minification filter
        mResult = mDevice->SetSamplerState(i, D3DSAMP_MINFILTER, D3DTEXF_POINT);
        assert(mResult == D3D_OK);

        // Disable mipmapping in general -- We don't want mipmapping on the sky
        // box so the seems will join perfectly
        mResult = mDevice->SetSamplerState(i, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
        assert(mResult == D3D_OK);
    }
}
