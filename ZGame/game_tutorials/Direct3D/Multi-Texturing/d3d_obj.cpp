...
void CD3DObj::setDefaultRenderState()
{
    ...
    // By paramter:
    // D3DADAPTER_DEFAULT -- Which device (ie graphics card) to get
    //						 the capabilites for.  This flag gets the
    //						 primary display adapter of the computer.
    // D3DDEVTYPE_HAL -- The device type.  We specified a hardware device
    //					 in when we created the device, thus we use the
    //					 same flag here.
    // &caps -- Address of a D3D9CAPS struct, to hold all of the device capabilites data
    ...
    {
        // The SetSamplerState() method is used to primarily set the UV wrapping
        // modes and magnification and minification methods used when mipmapping per
        // texture stage.  Here we are going to set magnification and minification methods
        // when mipmapping to be linear.  This means that bilinear interpolation filtering
        // is used to determine the resultant pixel color per mipmap.  Then trilinear filtering
        // is used to blend the two colors from the two nearest mipmaps.

        // By paramteter:
        // i -- The texture stage to set the sampler state for
        // D3DSAMP_MAGFILTER -- Sampler state to modify
        // D3DTEXF_LINEAR -- Value to modify sampler state to
...
// Sets the rendering state of a texture, specified by "state" to "value" at the
// texture stage specified by "stage"
bool CD3DObj::setTextureState(int stage, D3DTEXTURESTAGESTATETYPE state, DWORD value)
{
    // By parameter:
    // stage -- The index of the texture blend stage to set
    // state -- Texture state to set
    // value -- The value to set the texture state to
    return (mDevice->SetTextureStageState(stage, state, value) == D3D_OK);
}
