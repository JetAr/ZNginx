...
void CD3DObj::setDefaultRenderState()
{
    ...
    // Set source blend flag to use source alpha
    // **NOTE** Without this flag set the water will not blend
    mResult = mDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
    assert(mResult == D3D_OK);

    // Set destination blend flag to use inverse source alpha
    // **NOTE** Without this flag set the water will not blend
    mResult = mDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
    assert(mResult == D3D_OK);

    // Set the alpha blending operation, for texture stage 0, to use the alpha value of the
    // diffuse color at each vertex
    // **NOTE** Without this flag set the water will not blend
    mResult = mDevice->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_BLENDDIFFUSEALPHA);
    assert(mResult == D3D_OK);
...
// Sets the fill mode of the renderer
bool CD3DObj::setFillMode(D3DFILLMODE fillMode)
{
    mResult = mDevice->SetRenderState(D3DRS_FILLMODE, fillMode);
    return (mResult == D3D_OK);
}

// Sets the alpha blend mode
bool CD3DObj::setAlphaBlendMode(bool onOrOff)
{
    mResult = mDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, onOrOff);
    return (mResult == D3D_OK);
}
...
void CD3DObj::setPresentationParams(D3DPRESENT_PARAMETERS &params)
{
    ...
    params.SwapEffect = D3DSWAPEFFECT_FLIP; // Flip the back buffer
    params.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE; // Immediate update
    params.EnableAutoDepthStencil = TRUE; // We want a Z-buffer
    params.AutoDepthStencilFormat = D3DFMT_D16; // 16-bit Z-buffer
    ...
    else
    {
        ...
        params.BackBufferFormat = D3DFMT_UNKNOWN; // User current desktop color format
    }
}
