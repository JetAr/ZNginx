...
/*
	This is our vertex.  At the bottom is a quick reference to the order a D3D vertex
	must be in.
*/

// The define for our vertex type
// D3DFVF_XYZ -- It contains a world (x,y,z) position
// D3DFVF_DIFFUSE -- It contains an ARGB color
// (1 << D3DFVF_TEXCOUNT_SHIFT) -- It contains one set of UV's
#define SVertexType D3DFVF_DIFFUSE | D3DFVF_XYZ | (1 << D3DFVF_TEXCOUNT_SHIFT)

// Our vertex struct
struct SVertex
{
    float x, y, z; // Our position in space (world coordinates)
    DWORD color; // Our color
