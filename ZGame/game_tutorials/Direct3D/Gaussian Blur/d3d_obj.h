...
public:
    ...
    // If technique is NULL, uses the default "SAQ" technique for rendering
    // otherwise uses supplied technique
    static bool renderSAQ(float x, float y, float wid, float hgt, const char *technique = NULL);
...
private:
    ...
    // Vertex declarations
    static IDirect3DVertexDeclaration9 *mSAQVertexDecl; // Vertex declaration for screen aligned quad
