/*	Done by TheTutor
  ...
  The order of precedence (operation precedence) is something any good programmer needs to
  know and know well.  So what exactly are we talking about with "order of precedence"?
  ...
  So I'm going to list some precedence rules.  In main() there are "10 chunks of code"
  where "the order of precedence" will determine the final "answer".  I suggest looking at them
  all and figuring out (on paper) what will be printed.  To see what is printed simply change
  ...
  But first, the precedence rules:
	...
	4)  Prefix operators will execute BEFORE any multiplication, division,
		...
		++X -- This is the prefix increment operator, it increases X's value by one.
		X++ -- This is the postfix increment operator, it also increases X's value by one.
  ...
  It's time to see what ya know :)  I HIGHLY SUGGEST DOING THIS ON PAPER FIRST.  IT'S AN
...
int main()
{
    ...
    if(SHOW_ANSWERS == 0)
    {
        printf("Change the SHOW_ANSWERS to 1 in the code to see the answers.\n");
        return 0;
    }
...
//************************************************************** Number 1
    ...
    if(SHOW_ANSWERS)
        printf("The answer equals %d\n\n",answer);

//************************************************************** Number 2

    answer = (10 + (5 + 5) * (20 / 2) - (50 - (50 / 2)));
...
/*
	...
	MSDN at http://msdn.microsoft.com/library/
	...
	As always if you want/need more help post your questions at www.GameTutorials.com.
*/
