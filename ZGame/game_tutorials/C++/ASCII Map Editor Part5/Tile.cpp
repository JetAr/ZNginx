...
// In this tutorial version we added 2 new functions that return a tile, given a position,
// along with a way to set a tile on the map to solid (for collision).
//
// SetCollisionTile() and FindMapTile().
//
// We also added some additions to a couple functions to insert the action
// keys and display more information about our tiles in the prompt.
//
//
...
CTile::CTile():m_type(0),m_bSolid(false),m_actionKey(0)		// *** NEW *** Init new variables
{
...
///////////////////////////// FIND MAP TILE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This returns a tile (any type of tile) at the x and y position
/////
///////////////////////////// FIND MAP TILE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

CTile *CMap::FindMapTile(int mapX, int mapY)
{
    // In this function we just go through the appropriate list and then
    // check each tile to see if it matches the position passed in.  This is
    // very similar to our DeleteTile() function's code.

    if(m_currentTypeFlag == TILE_TYPE)				// If the current tile type is a tile
    {
        // Go through all of the tiles on our map from the back to the front of the list
        for(int i = (int)m_vTiles.size() - 1; i >= 0; i--)
        {
            ...
            {
                // Return a pointer to the correct tile
                return 	&m_vTiles[i];
            }
        }
    }
    else if(m_currentTypeFlag == ITEM_TYPE)				// If the current tile type is an item
    {
        ...
        for(int i = (int)m_vItems.size() - 1; i >= 0; i--)
        {
            ...
            {
                // Return a casted pointer to the correct item
                return 	(CTile*)&m_vItems[i];
            }
        }
    }
    else if(m_currentTypeFlag == MONSTER_TYPE)		// If the current tile type is an monster
    {
        ...
        for(int i = (int)m_vMonsters.size() - 1; i >= 0; i--)
        {
            ...
            {
                // Return a casted pointer to the correct monster
                return (CTile*)&m_vMonsters[i];
            }
        }
    }
    else if(m_currentTypeFlag == NPC_TYPE)			// If the current tile type is an npc
    {
        ...
        for(int i = (int)m_vNpcs.size() - 1; i >= 0; i--)
        {
            ...
            {
                // Return a casted pointer to the correct npc
                return (CTile*)&m_vNpcs[i];
            }
        }
    }
    else if(m_currentTypeFlag == EXIT_TYPE)			// If the current tile type is an exit
    {
        ...
        for(int i = (int)m_vExits.size() - 1; i >= 0; i--)
        {
            ...
            {
                // Return a casted pointer to the correct exit
                return (CTile*)&m_vExits[i];
            }
        }
    }

    // If there is no tile found, return a null
    return NULL;
}
...
///////////////////////////// DRAW TILE INFO \\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This draws a prompt that displays the current tile's info
/////
///////////////////////////// DRAW TILE INFO \\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

void CMap::DrawTileInfo()
{
    // Get a handle to our console window and set our cursor position for our prompt
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
    ...
    // We added a couple checks to this function so that we could display more
    // information about our tiles.  We display the tiles that we can cycle through
    // that are attached to our cursor.  We also wanted to see the details of a tile
    // placed on the map, in case we wanted to delete a tile and you didn't remember
    // which tile was which.

    // Get the current type
    int type = g_Map.GetCurrentType();

    // If we have a valid cursor tile we want to display info about the cursor tile
    if(g_pCursorTile)
    {
        if(type == TILE_TYPE)				// If the tile is a tile
        {
            // Display whether or not the cursor tile is solid
            if(g_pCursorTile->IsSolid())
                cout << "Current tile is solid";
            else
                cout << "Current tile is not solid";
        }
        else if(type == ITEM_TYPE)		// If the tile is an item
        {
            // Cast the cursor tile as an item, then extract it's name
            CItem *pItem = (CItem*)g_pCursorTile;
            cout << "(Space Bar Cycles) Current tile: " << pItem->GetItemName();
        }
        else if(type == MONSTER_TYPE)		// If the tile is a monster
        {
            // Cast the cursor tile as a monster, then extract it's name
            CMonster *pMonster = (CMonster*)g_pCursorTile;
            cout << "(Space Bar Cycles) Current tile: " << pMonster->GetName();
        }
        else if(type == NPC_TYPE)			// If the tile is a npc
        {
            // Cast the cursor tile as a npc, then extract it's name
            CNpc *pNpc = (CNpc*)g_pCursorTile;
            cout << "(Space Bar Cycles) Current tile: " << pNpc->GetName();
        }
    }
    else
    {
        // If there is no tile under our cursor that is the same as our mode, return
        if(!FindMapTile(g_cursorPos.X, g_cursorPos.Y)) return;

        if(type == TILE_TYPE)				// If the tile is a tile
        {
            // Find the tile under our cursor and print it's info in the prompt
            CTile *pTile = FindMapTile(g_cursorPos.X, g_cursorPos.Y);
            cout << "Tile: (" << pTile->GetIndex().X << "," << pTile->GetIndex().Y;
            cout << ") Solid: " << pTile->IsSolid() << " Action#: " << pTile->GetActionKey();
        }
        else if(type == ITEM_TYPE)		// If the tile is an item
        {
            // Cast the cursor tile as an item, then print it's info in the prompt
            CItem *pItem = (CItem*)FindMapTile(g_cursorPos.X, g_cursorPos.Y);
            cout << "Item: " << pItem->GetItemName() << " Action#: " << pItem->GetActionKey();
        }
        else if(type == MONSTER_TYPE)		// If the tile is a monster
        {
            // Cast the cursor tile as a monster, then print it's info in the prompt
            CMonster *pMonster = (CMonster*)FindMapTile(g_cursorPos.X, g_cursorPos.Y);
            cout << "Monster: " << pMonster->GetName() << " Action#: " << pMonster->GetActionKey();
        }
        else if(type == NPC_TYPE)			// If the tile is a npc
        {
            // Cast the cursor tile as a npc, then print it's info in the prompt
            CNpc *pNpc = (CNpc*)FindMapTile(g_cursorPos.X, g_cursorPos.Y);
            cout << "Npc: " << pNpc->GetName() << " Action#: " << pNpc->GetActionKey();
            cout << " Message: " << pNpc->GetMessage();
        }
        else if(type == EXIT_TYPE)			// If the tile is a npc
        {
            // Cast the cursor tile as a npc, then print it's info in the prompt
            CExit *pExit = (CExit*)FindMapTile(g_cursorPos.X, g_cursorPos.Y);
            cout << "Exit to: " << pExit->GetNewMap() << " Action#: " << pExit->GetActionKey();
            cout << " Position: (" << pExit->GetNewPos().X << "," << pExit->GetNewPos().X << ")";
        }
    }
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
        exit.SetActionKey(pTile->GetActionKey());	// Set the exit action key
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
...
///////////////////////////// SET COLLISION TILE \\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This goes through all the tiles and finds the tile with matching x,y
/////
///////////////////////////// SET COLLISION TILE \\\\\\\\\\\\\\\\\\\\\\\\\\\\*

void CMap::SetCollisionTile(bool bIsSolid, int x, int y)
{
    // Go through all of our normal tiles
    for(int i = 0; i < (int)m_vTiles.size(); i++)
    {
        // Grab the current index in our tile array
        COORD index = m_vTiles[i].GetIndex();

        // Check if the current tile has the same position as the desired position
        if((x == index.X) && (y == index.Y))
        {
            // Set this tile as solid and quit this function
            m_vTiles[i].SetSolidValue(bIsSolid);
            break;
        }
    }
}
...
// *Quick Notes*
//
// We added some extra code to display more data about our tiles
// in the prompt, as well as functions to set tiles as solid and
// get a tile that matches the same position as our cursor.
//
//
