...
// Now that we have party members we need to write functions that will handle it.
// This includes the movement of the party, as well as the drawing of the party.
// Since we can attack monsters now, we needed to add an Attack() function.  Every
// player in the party can attack a monster if they run into them.  We also need
// to check if a party member died.  We also need to free our party members at
// the end of the game, since they were allocated with the "new" command.  We did
// this in the CPlayer class deconstructor.
//
//
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
    ...
    // Because our party is added dynamically with "new", we need to free the
    // party members (other than the main player since it's a global object).
...
void CPlayer::Attack(CMonster *pMonster)
{
    ...
    // This function is very similar to the attack function for the monster,
    // but we need to deal with new levels, experience and gold.  We also don't
    // need to loop through each player, just 1 player will attack.
    ...
    // Now comes the rewarding part!  We need to check if the monster was killed.  If they
    // were, then we can add experience, gold, levels, and possibly new stats to the player
    if(pMonster->GetLife() <= 0)
    {
        ...
        // Now we increase the player's experience by the strength of the monster killed.
        // We then can delete the monster from the map so it doesn't keep fighting dead :)
        ...
        // Below is where we find out if we gained a level or not.  We start with
        // 1 level, and then do some math to see if our experience is enough to
        // justify that level.  If it is, we increase i and try for a new level.
        while(1)
        {
...
// *Quick Notes*
//
// In this file we added 3 new functions to handle attacking monsters
// and handling the movement and drawing of our party members.  We
// also added some code in our CPlayer class deconstructor to free the
// allocated party members.
//
// Let's take a look now at ActionKeys.cpp to see how the cut scenes
// and the action keys work.
//
//
