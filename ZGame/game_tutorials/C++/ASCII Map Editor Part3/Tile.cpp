...
// In this tutorial version we add 2 new functions:
//
// Load() and save()
//
// These of course... allow us to load and save :)  This is all that was
// added to this file.  We just implement the functionality to load in our
// map data, as well as save maps to be loaded in later, be it a game or the editor.
//
//
...
void CMap::Load(char *szFile)
{
    ...
    // This is where all the loading magic happens :)  Like I mentioned before
    // in Main.cpp, we load our file by first loading in the header information,
    // which stores how many tiles we have in each list.  We then do a loop
    // to read in each tile.  There is nothing complicated about this function.
    ...
    // Here we initialize all of our variables for the header.  We could add
    // all of these variables into a structure or class called tHeader, and then
    // do one fread() to read all of the data at the same time.  As your header
    // gets more complicated and/or larger, this is a really good idea.
...
{
    // Before we do any saving, we want to make sure that the file name
    // passed in has our .map extension.  If it doesn't, we need to add it.
    // Next, we create a new file to write (save) in binary mode.  Then we
    // save the header, which has all of the list sizes, then go through and
    // save each tile in the appropriate order.
...
// *Quick Notes*
//
// There was only 2 functions added to this file, Save() and Load().
