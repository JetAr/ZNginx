...
void CMap::Load(const char *szFile)
{
    ...
    // Handle the events for the maps, if any
...
void CMap::Draw()
{
    ...
    // Draw and check each player in the part
...
void CMap::MoveMonsters()
{
    ...
    for(int i = 0; i < (int)m_vMonsters.size(); i++)
    {
        ...
        if((rand() % 50000000) == 2500)
        {
            ...
            // If the monster didn't collide with sometime, set their new position
            if(!IsACollision(position.X, position.Y))
            {
...
void CMonster::Attack(CPlayer *pPlayer)
{
    ...
    // Here we check who is stronger, the player or the monster, then adjust accordingly
