...
template <class charT>
inline bool __is_basic_char_type(charT *)
{
    return false;
}
template <class charT>
inline bool __is_one_byte_char_type(charT *)
{
    return false;
}

inline bool __is_basic_char_type(char *)
{
    return true;
}
inline bool __is_one_byte_char_type(char *)
{
    return true;
}
inline bool __is_basic_char_type(wchar_t *)
{
    return true;
}
...
inline void __cond_store_eos(char& c)
{
    c = 0;
}
inline void __cond_store_eos(wchar_t& c)
{
    c = 0;
}
...
#   endif
    ...
    operator __rope_RopeBase<charT,Alloc> *()
    {
        return ptr;
    }
...
#       endif
    ...
    typedef simple_alloc<__rope_RopeFunction<charT,Alloc>, Alloc> FAlloc;
    typedef simple_alloc<__rope_RopeSubstring<charT,Alloc>, Alloc> SAlloc;
    static bool is0(charT c)
    {
        return c == __eos((charT *)0);
    }
    enum { copy_max = 23 };
...
inline void swap(crope x, crope y)
{
    x.swap(y);
}
inline void swap(wrope x, wrope y)
{
    x.swap(y);
}
